...
class AssertionHelper:
    ...
    @staticmethod
    def assert_response_structure(data: Union[Response, Dict], required_fields: List[str]):
        ...
        for field in required_fields:
            assert field in data, f"Required field '{field}' not found in response: {data}"
    ...
    for path, methods in paths.items():
        ...
        for category, patterns in API_ENDPOINTS.items():
            for pattern in patterns:
                if path == pattern or path.startswith(pattern.replace("{", "").replace("}", "")):
...
def authenticated_user(api_client, test_data_manager) -> Tuple[str, Dict]:
    ...
    assert response.status_code in [200, 201], f"Failed to create/authenticate user: {response.text}"
...
def authenticated_admin(api_client, test_data_manager) -> Tuple[str, Dict]:
    ...
    assert response.status_code in [200, 201], f"Failed to create/authenticate admin: {response.text}"
...
@pytest.fixture(scope="function")
def test_conversation(api_client, authenticated_user, test_assistant, test_data_manager) -> Dict:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "blackbox: mark test as blackbox API test"
    )
    config.addinivalue_line(
        "markers", "authentication: mark test as authentication test"
    )
    config.addinivalue_line(
        "markers", "users: mark test as user management test"
    )
    config.addinivalue_line(
        "markers", "assistants: mark test as assistant management test"
    )
    config.addinivalue_line(
        "markers", "conversations: mark test as conversation test"
    )
    config.addinivalue_line(
    ...
    config.addinivalue_line(
        "markers", "ai: mark test as AI service test"
    )
    config.addinivalue_line(
