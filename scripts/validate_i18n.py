...
class I18nValidator:
                ...
                if file_path.exists():
                    try:
                        with open(file_path, encoding="utf-8") as f:
                            json.load(f)
                        print(
                            f"✅ {file_path.relative_to(self.project_root)} - Valid JSON"
                        )
    ...
    def validate_key_consistency(self) -> bool:
        ...
        for directory_name, directory in [
            ("Frontend", self.frontend_i18n_dir),
            ("Backend", self.backend_i18n_dir),
        ]:
            ...
            for lang, data in translations.items():
                ...
                if missing:
                    self.errors.append(
                        f"{directory_name} {lang}: Missing keys: {sorted(missing)}"
                    )
                    success = False
                ...
                if extra:
                    self.warnings.append(
                        f"{directory_name} {lang}: Extra keys: {sorted(extra)}"
                    )
    ...
    def check_missing_translations(self) -> bool:
        ...
        for directory_name, directory in [
            ...
            for lang, data in translations.items():
                ...
                if missing:
                    self.warnings.append(
                        f"{directory_name} {lang}: Empty translations: {sorted(missing)}"
                    )
                else:
    ...
    def validate_translation_usage(self) -> bool:
        ...
        if "en" in frontend_translations:
            ...
            if unused:
                self.warnings.append(
                    f"Frontend: Unused translation keys: {sorted(unused)}"
                )
            ...
            if missing:
                self.errors.append(
                    f"Frontend: Used but not defined keys: {sorted(missing)}"
                )
                success = False

            print(
                f"✅ Frontend: {len(frontend_used_keys)} keys used, {len(unused)} unused"
            )
