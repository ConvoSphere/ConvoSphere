...
from saml2.saml import Attribute, AttributeValue, NameID

from app.services.saml_service import SAMLService, MockSamlSettings
...
class TestSAMLService:
    ...
    def mock_settings(self):
        ...
        settings.sso_saml_acs_url = "http://localhost:8000/api/v1/auth/sso/callback/saml"
        return settings
    ...
    def test_setup_saml_client_success(self, mock_xmlsec, saml_service, mock_settings):
        ...
        with patch.object(saml_service, '_create_temp_certificates') as mock_cert:
            ...
            with patch('app.services.saml_service.Saml2Config') as mock_config_class:
                with patch('app.services.saml_service.Saml2Client') as mock_client_class:
    ...
    def test_create_temp_certificates(self, mock_tempfile, saml_service):
        ...
        with patch('app.services.saml_service.x509') as mock_x509:
            with patch('app.services.saml_service.rsa') as mock_rsa:
                with patch('app.services.saml_service.serialization') as mock_serialization:
                    # Mock private key
    ...
    @pytest.mark.asyncio
    async def test_get_login_url_success(self, saml_service, mock_settings, mock_saml_client):
        """Test successful login URL generation."""
        saml_service.settings = mock_settings
        saml_service.saml_client = mock_saml_client
        
        # Mock the IdP entity ID method
        saml_service._get_idp_entity_id = MagicMock(return_value="http://idp.example.com")
        ...
        mock_saml_client.apply_binding.return_value = ("POST", "https://idp.example.com/login")
    ...
    @pytest.mark.asyncio
    async def test_get_login_url_failure(self, saml_service, mock_settings, mock_saml_client):
        """Test login URL generation failure."""
        saml_service.settings = mock_settings
        saml_service.saml_client = mock_saml_client
        
        # Mock the IdP entity ID method to raise an exception
        saml_service._get_idp_entity_id = MagicMock(side_effect=Exception("IdP not found"))
    ...
    @pytest.mark.asyncio
    async def test_handle_saml_response_success(self, saml_service, mock_settings, mock_saml_client):
    ...
    def test_get_metadata_success(self, saml_service, mock_saml_client):
        ...
        with patch('app.services.saml_service.create_metadata_string') as mock_create_metadata:
...
class TestMockSamlSettings:
    ...
    def test_mock_settings_initialization(self):
        ...
        assert settings.sso_saml_acs_url == "http://localhost:8000/api/v1/auth/sso/callback/saml"
