...
class TestOAuthService:
    ...
    def test_is_provider_enabled(self, oauth_service):
        ...
        with patch.object(oauth_service.settings, 'sso_google_enabled', True):
            with patch.object(oauth_service.settings, 'sso_google_client_id', 'test-id'):
                with patch.object(oauth_service.settings, 'sso_google_client_secret', 'test-secret'):
    ...
    def test_get_redirect_uri(self, oauth_service):
        ...
        with patch.object(oauth_service.settings, 'sso_google_redirect_uri', 'http://localhost/callback'):
            assert oauth_service._get_redirect_uri('google') == 'http://localhost/callback'
    ...
    async def test_get_authorization_url_google(self, oauth_service):
        ...
        with patch.object(oauth_service, '_is_provider_enabled', return_value=True):
            with patch.object(oauth_service.oauth, 'create_client') as mock_create_client:
                ...
                mock_client.authorize_redirect = AsyncMock(return_value='http://google.com/auth')
                mock_create_client.return_value = mock_client
                
                result = await oauth_service.get_authorization_url('google', mock_request)
    ...
    async def test_handle_callback_google(self, oauth_service, mock_user_info):
        ...
        with patch.object(oauth_service, '_is_provider_enabled', return_value=True):
            with patch.object(oauth_service.oauth, 'create_client', return_value=mock_client):
                mock_client.authorize_access_token = AsyncMock(return_value=mock_token)
                
                with patch.object(oauth_service, '_get_google_user_info', return_value=mock_user_info):
    ...
    @pytest.mark.asyncio
    async def test_process_sso_user_new_user(self, oauth_service, mock_user_info, db_session):
        """Test processing new SSO user."""
        with patch.object(oauth_service, '_get_auth_provider', return_value=AuthProvider.OAUTH_GOOGLE):
            with patch.object(UserService, 'get_user_by_external_id', return_value=None):
                with patch.object(UserService, 'create_sso_user') as mock_create:
                    mock_user = MagicMock()
                    mock_user.id = 'test-user-id'
                    mock_create.return_value = mock_user
                    
                    result = await oauth_service.process_sso_user(mock_user_info, 'google', db_session)
                    assert result == mock_user

    @pytest.mark.asyncio
    async def test_process_sso_user_existing_user(self, oauth_service, mock_user_info, db_session):
        """Test processing existing SSO user."""
        mock_user = MagicMock()
        mock_user.id = 'test-user-id'
        
        with patch.object(oauth_service, '_get_auth_provider', return_value=AuthProvider.OAUTH_GOOGLE):
            with patch.object(UserService, 'get_user_by_external_id', return_value=mock_user):
                result = await oauth_service.process_sso_user(mock_user_info, 'google', db_session)
                assert result == mock_user
    ...
    async def test_create_sso_tokens(self, oauth_service):
        ...
        with patch.object(oauth_service.settings, 'jwt_access_token_expire_minutes', 30):
...
class TestSSOEndpoints:
    ...
    async def test_sso_login_google(self, client):
        ...
        with patch('app.services.oauth_service.oauth_service.get_authorization_url') as mock_auth:
    ...
    async def test_sso_callback_success(self, client, db_session):
        ...
        with patch('app.services.oauth_service.oauth_service.handle_callback') as mock_callback:
            with patch('app.services.oauth_service.oauth_service.process_sso_user') as mock_process:
                with patch('app.services.oauth_service.oauth_service.create_sso_tokens') as mock_tokens_func:
                    mock_callback.return_value = {'user_info': mock_user_info, 'token': {}, 'provider': 'google'}
    ...
    def test_sso_callback_error(self, client):
        ...
        with patch('app.services.oauth_service.oauth_service.handle_callback', side_effect=Exception("OAuth error")):
            response = client.get("/api/v1/auth/sso/callback/google")
            assert response.status_code == 400
...
class TestSSOUserService:
    ...
    def test_get_user_by_external_id_not_found(self, user_service, db_session):
        ...
        user = user_service.get_user_by_external_id('nonexistent', AuthProvider.OAUTH_GOOGLE)
        assert user is None
