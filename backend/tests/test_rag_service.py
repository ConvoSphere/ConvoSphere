...
from app.core.exceptions import AIError, ConfigurationError, ValidationError
from app.schemas.rag import (
    RAGConfig, RAGRequest, RAGResponse, RAGResult, RAGMetrics,
    RAGStrategy, ContextRankingMethod, EmbeddingModel
)
...
class TestRAGService:
    ...
    async def test_initialize_success(self, rag_service):
        ...
        with patch.object(rag_service.weaviate_service, 'health', return_value=True):
            with patch.object(rag_service, '_cache_enabled', True):
    ...
    async def test_initialize_cache_failure(self, rag_service):
        ...
        with patch.object(rag_service.weaviate_service, 'health', return_value=True):
            with patch('app.services.rag_service.cache_service.initialize', side_effect=Exception("Cache error")):
                await rag_service.initialize()
                assert rag_service._cache_enabled is False
    ...
    async def test_semantic_retrieval(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service.weaviate_service, 'semantic_search_knowledge', return_value=mock_knowledge_results):
            with patch.object(rag_service.weaviate_service, 'semantic_search_messages', return_value=[]):
    ...
    async def test_hybrid_retrieval(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service, '_semantic_retrieval', return_value=[{'content': 'semantic'}]):
            with patch.object(rag_service, '_keyword_retrieval', return_value=[{'content': 'keyword'}]):
                results = await rag_service._hybrid_retrieval(sample_request, sample_config, [])
                
                assert len(results) == 2
                assert any(r['content'] == 'semantic' for r in results)
                assert any(r['content'] == 'keyword' for r in results)
    ...
    async def test_keyword_retrieval(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service.weaviate_service, 'semantic_search_knowledge', return_value=mock_results):
            ...
            assert all(r['source'] == 'knowledge_base' for r in results)
    ...
    async def test_contextual_retrieval(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service.weaviate_service, 'semantic_search_knowledge', return_value=mock_results):
            ...
            assert len(results) == 1
            assert results[0]['content'] == 'contextual result'
    ...
    async def test_adaptive_retrieval_technical(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service, '_semantic_retrieval', return_value=[{'content': 'technical result'}]):
            ...
            assert len(results) == 1
            assert results[0]['content'] == 'technical result'
    ...
    async def test_adaptive_retrieval_conversational(self, rag_service, sample_request, sample_config):
        ...
        with patch.object(rag_service, '_contextual_retrieval', return_value=[{'content': 'conversational result'}]):
            ...
            assert len(results) == 1
            assert results[0]['content'] == 'conversational result'
    ...
    async def test_process_results(self, rag_service, sample_request, sample_config):
        ...
        assert results[0].ranking_score >= results[1].ranking_score  # Should be sorted by ranking
    ...
    def test_build_contextual_query(self, rag_service):
        ...
        assert 'help with database' in contextual_query
        assert 'PostgreSQL' in contextual_query
        assert 'connection' in contextual_query
    ...
    def test_analyze_query(self, rag_service):
        ...
        assert technical_analysis['is_technical'] is True
        assert conversational_analysis['is_conversational'] is True
        assert specific_analysis['has_specific_terms'] is True
    ...
    def test_combine_results(self, rag_service):
        ...
        assert any(r['content'] == 'unique content 1' for r in combined)
        assert any(r['content'] == 'unique content 2' for r in combined)
    ...
    async def test_create_config(self, rag_service, sample_config):
        ...
        assert any(cid == config_id for cid, _ in configs)
    ...
    async def test_delete_config(self, rag_service, sample_config):
        ...
        configs = await rag_service.list_configs()
        assert not any(cid == config_id for cid, _ in configs)
    ...
    async def test_cache_operations(self, rag_service):
        ...
        # Test cache set
        with patch.object(rag_service, '_cache_enabled', True):
            with patch('app.services.rag_service.cache_service.set', return_value=True):
                await rag_service._cache_result(cache_key, test_data)
        
        # Test cache get
        with patch.object(rag_service, '_cache_enabled', True):
            with patch('app.services.rag_service.cache_service.get', return_value=test_data):
    ...
    def test_calculate_content_similarity(self, rag_service):
        ...
        assert similarity1 > similarity2  # Similar content should have higher similarity
    ...
    def test_format_knowledge_results(self, rag_service):
        ...
        assert formatted[0]['source'] == 'knowledge_base'
        assert formatted[0]['source_type'] == 'document'
        assert formatted[0]['source_id'] == '1'
        assert formatted[0]['similarity_score'] == 0.8
    ...
    def test_format_conversation_results(self, rag_service):
        ...
        assert len(formatted) == 1
        assert formatted[0]['content'] == 'conversation content'
        assert formatted[0]['source'] == 'conversation'
        assert formatted[0]['source_type'] == 'message'
        assert formatted[0]['source_id'] == 'msg1'
        assert formatted[0]['similarity_score'] == 0.7