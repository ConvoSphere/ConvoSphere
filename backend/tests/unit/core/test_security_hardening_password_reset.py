...
class TestPasswordResetRateLimiting:
    ...
    def test_rate_limit_password_reset_by_ip(self, security_validator):
        ...
        for i in range(5):
            assert (
                security_validator.rate_limit_password_reset_by_ip(ip_address) is True
            )
    ...
    def test_rate_limit_password_reset_custom_limits(self, security_validator):
        ...
        for i in range(10):
            assert (
                security_validator.rate_limit_password_reset_by_ip(
                    ip_address, max_requests=10
                )
                is True
            )
