...
class TestComplexityAnalyzer:
    ...
    def test_analyze_length_complex(self):
        ...
        message = "This is a very long message with many words that should trigger a higher complexity score"
        score = self.analyzer._analyze_length(message)
        assert score > 0.5
...
class TestAgentMemoryManager:
    ...
    def test_add_memory(self):
        ...
        assert memory.conversation_id == "test_conv"
...
class TestAgentReasoningEngine:
    ...
    def test_analyze_tool_relevance(self):
        ...
        relevance = self.reasoning_engine._analyze_tool_relevance(user_message, available_tools)
...
class TestHybridModeManager:
    ...
    def test_get_state(self):
        ...
        state = self.manager.get_state("test_conv")
    ...
    async def test_decide_mode_simple_query(self):
        ...
        assert decision.conversation_id == "test_conv"
        assert decision.recommended_mode in [ConversationMode.CHAT, ConversationMode.AUTO]
    ...
    async def test_decide_mode_complex_query(self):
        ...
        assert decision.conversation_id == "test_conv"
        assert decision.recommended_mode in [ConversationMode.AGENT, ConversationMode.AUTO]
    ...
    def test_cleanup_conversation(self):
        ...
        assert self.manager.get_state("test_conv") is not None
        
        # Clean up
        self.manager.cleanup_conversation("test_conv")
        
        # Verify conversation was removed
        assert self.manager.get_state("test_conv") is None

    def test_get_stats(self):
        """Test statistics retrieval."""
        # Initialize some conversations
        self.manager.initialize_conversation("conv1", "user1")
        self.manager.initialize_conversation("conv2", "user2")
    ...
    def test_determine_recommended_mode_complexity(self):
        ...
        state = self.manager.initialize_conversation("test_conv", "test_user")
    ...
    def test_calculate_confidence(self):
        ...
        from app.schemas.hybrid_mode import AgentReasoning
...
class TestHybridModeManagerIntegration:
    ...
    async def test_full_workflow(self):
        ...
        final_state = self.manager.get_state("workflow_test")
        
        # Assertions
        assert decision1.recommended_mode in [ConversationMode.CHAT, ConversationMode.AUTO]
        assert decision2.recommended_mode in [ConversationMode.AGENT, ConversationMode.AUTO]
        assert decision2.complexity_score > decision1.complexity_score
        assert response.success is True
