...
from app.models.knowledge import Document, Tag, DocumentProcessingJob
...
@pytest.fixture
def test_document_with_tags(db_session: Session, test_user: User, test_tags: list[Tag]) -> Document:
...
@pytest.fixture
def test_processing_job(db_session: Session, test_user: User, test_document: Document) -> DocumentProcessingJob:
...
@pytest.fixture
def test_processing_jobs(db_session: Session, test_user: User, test_document: Document) -> list[DocumentProcessingJob]:
...
def mock_weaviate_service():
    ...
    with pytest.MonkeyPatch().context() as m:
        ...
        m.setattr("app.services.knowledge_service.WeaviateService", lambda: mock_service)
        yield mock_service
...
def mock_background_job_service():
    ...
    with pytest.MonkeyPatch().context() as m:
        ...
        m.setattr("app.services.background_job_service.background_job_service", mock_service)
        yield mock_service
