...
from cli.utils.helpers import get_backup_dir, format_file_size, confirm_action
...
class BackupCommands:
    ...
    def restore(self, backup_file: str, confirm: bool = False) -> None:
        ...
        try:
            ...
            if not confirm:
                if not confirm_action(f"Are you sure you want to restore from '{backup_file}'? This will overwrite current data."):
    ...
    def list(self, backup_dir: str = None) -> None:
        ...
        try:
            ...
            for backup in backup_files:
                print(f"{backup['name']:<30} {format_file_size(backup['size']):<12} {backup['time'].strftime('%Y-%m-%d %H:%M:%S'):<20}")
