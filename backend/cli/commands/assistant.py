...
class AssistantCommands:
    ...
    def list(self) -> None:
        ...
        try:
            ...
            print("-" * 80)
            print(f"{'ID':<10} {'Name':<20} {'Model':<15} {'Status':<10} {'Created':<20}")
            print("-" * 80)
            
            for assistant in assistants:
                status = "Active" if assistant.is_active else "Inactive"
                created = assistant.created_at.strftime("%Y-%m-%d %H:%M") if assistant.created_at else "Unknown"
                print(f"{assistant.id:<10} {assistant.name:<20} {assistant.model:<15} {status:<10} {created:<20}")
    ...
    def create(self) -> None:
        ...
        try:
            ...
            existing_assistant = db.query(Assistant).filter(Assistant.name == name).first()
    ...
    def delete(self, assistant_id: str, confirm: bool = False) -> None:
        ...
        try:
            ...
            if not confirm:
                if not confirm_action(f"Are you sure you want to delete assistant '{assistant_id}'?"):
