...
from sqlalchemy import JSON, Column, ForeignKey, String, Text, Boolean, DateTime, Integer, Table
...
domain_group_members = Table(
    ...
    Base.metadata,
    Column("domain_group_id", UUID(as_uuid=True), ForeignKey("domain_groups.id"), primary_key=True),
    Column("user_id", UUID(as_uuid=True), ForeignKey("users.id"), primary_key=True),
    Column("access_level", SQLEnum(AccessLevel), default=AccessLevel.READ_WRITE, nullable=False),
...
domain_group_hierarchy = Table(
    ...
    Base.metadata,
    Column("parent_domain_id", UUID(as_uuid=True), ForeignKey("domain_groups.id"), primary_key=True),
    Column("child_domain_id", UUID(as_uuid=True), ForeignKey("domain_groups.id"), primary_key=True),
    Column("relationship_type", String(50), default="contains", nullable=False),  # contains, collaborates, reports_to
...
class DomainGroup(Base):
    ...
    parent_domain_id = Column(UUID(as_uuid=True), ForeignKey("domain_groups.id"), nullable=True)
    
    # Organization and metadata
    organization_id = Column(UUID(as_uuid=True), nullable=True, index=True)
    external_id = Column(String(255), nullable=True, index=True)  # For external system integration
    ...
    # Access control
    default_access_level = Column(SQLEnum(AccessLevel), default=AccessLevel.READ_WRITE, nullable=False)
    ...
    def get_member_access_level(self, user_id: str) -> Optional[AccessLevel]:
    ...
    def can_access_resource(self, user_id: str, resource_type: ResourceType, required_level: AccessLevel = AccessLevel.READ_ONLY) -> bool:
        ...
        access_levels = [AccessLevel.READ_ONLY, AccessLevel.READ_WRITE, AccessLevel.FULL_ACCESS, AccessLevel.ADMIN, AccessLevel.OWNER]
...
domain_group_managers = Table(
    ...
    Column("domain_group_id", UUID(as_uuid=True), ForeignKey("domain_groups.id"), primary_key=True),
    Column("user_id", UUID(as_uuid=True), ForeignKey("users.id"), primary_key=True),
    Column("manager_type", String(50), default="manager", nullable=False),  # manager, admin, owner
...
class DomainResource(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    domain_group_id = Column(UUID(as_uuid=True), ForeignKey("domain_groups.id"), nullable=False)
    ...
    # Access control
    access_level = Column(SQLEnum(AccessLevel), default=AccessLevel.READ_WRITE, nullable=False)
