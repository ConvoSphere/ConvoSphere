...
from sqlalchemy import JSON, Column, ForeignKey, String, Text, Boolean, DateTime, Integer, Index
...
class ExtendedAuditLog(Base):
    ...
    def requires_immediate_attention(self) -> bool:
        ...
        return (
            ...
            self.threat_level == "critical" or
            self.risk_score and self.risk_score >= 80
        )
    ...
    def is_compliance_relevant(self) -> bool:
        ...
        return (
            self.compliance_frameworks is not None or
            self.data_classification in [DataClassification.CONFIDENTIAL, DataClassification.RESTRICTED, DataClassification.CLASSIFIED] or
            self.event_category in [AuditEventCategory.SECURITY, AuditEventCategory.COMPLIANCE]
        )
...
class AuditRetentionRule(Base):
    ...
    # Action settings
    action_on_expiry = Column(String(50), default="delete", nullable=False)  # delete, archive, anonymize
    notify_before_expiry = Column(Integer, nullable=True)  # Days before expiry to notify
...
class ComplianceReport(Base):
    ...
    # Status
    status = Column(String(50), default="draft", nullable=False)  # draft, review, approved, archived
...
class AuditArchive(Base):
    ...
    # Status
    status = Column(String(50), default="archiving", nullable=False)  # archiving, completed, failed
