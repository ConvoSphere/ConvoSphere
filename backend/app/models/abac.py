...
from sqlalchemy import JSON, Column, ForeignKey, String, Text, Boolean, DateTime, Integer
...
class ABACRule(Base):
    ...
    def evaluate(self, user: 'User', resource: Any = None, action: str = None, context: Dict[str, Any] = None) -> bool:
        """Evaluate ABAC rule."""
        context = context or {}
    ...
    def _extract_user_attributes(self, user: 'User') -> Dict[str, Any]:
        ...
        return {
            ...
            'groups': [{'id': str(group.id), 'name': group.name} for group in user.groups],
    ...
    def _extract_resource_attributes(self, resource: Any) -> Dict[str, Any]:
        ...
        if hasattr(resource, 'created_at'):
            attributes['created_at'] = resource.created_at.isoformat() if resource.created_at else None
        if hasattr(resource, 'updated_at'):
            attributes['updated_at'] = resource.updated_at.isoformat() if resource.updated_at else None
            
        return attributes
    
    def _extract_environment_attributes(self, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _apply_operator(self, operator: str, attribute_value: Any, expected_value: Any) -> bool:
        ...
        if operator == OperatorType.EQUALS:
            ...
            return attribute_value in expected_value if isinstance(expected_value, (list, tuple)) else False
        elif operator == OperatorType.NOT_IN:
            return attribute_value not in expected_value if isinstance(expected_value, (list, tuple)) else True
        elif operator == OperatorType.EXISTS:
            return attribute_value is not None
        elif operator == OperatorType.NOT_EXISTS:
            return attribute_value is None
        elif operator == OperatorType.REGEX:
            import re
            return bool(re.match(expected_value, str(attribute_value))) if attribute_value else False
...
class ABACPolicy(Base):
    ...
    def evaluate(self, user: 'User', resource: Any = None, action: str = None, context: Dict[str, Any] = None) -> Optional[bool]:
