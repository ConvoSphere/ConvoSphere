...
class IPWhitelistManager:
    ...
    def add_to_whitelist(self, ip_address: str, organization_id: Optional[str] = None) -> bool:
        """Add IP to whitelist."""
        key = f"{self.whitelist_prefix}{organization_id or 'global'}"
        return self.redis.sadd(key, ip_address) > 0
    
    def add_to_blacklist(self, ip_address: str, reason: str, duration: int = 3600) -> bool:
...
class ThreatDetector:
    ...
    def detect_anomalies(self, user: User, request: Request, action: str) -> List[SecurityEvent]:
    ...
    def _detect_login_anomalies(self, user: User, request: Request) -> List[SecurityEvent]:
    ...
    def _detect_permission_anomalies(self, user: User, request: Request, action: str) -> List[SecurityEvent]:
        """Detect permission-related anomalies."""
        events = []
        
        # Check for privilege escalation attempts
        if action in ['role_update', 'permission_grant'] and user.role not in ['admin', 'super_admin']:
            events.append(SecurityEvent(
    ...
    def _detect_behavioral_anomalies(self, user: User, request: Request) -> List[SecurityEvent]:
        ...
        if user_activity and 'normal_hours' in user_activity:
            ...
            if hour < 6 or hour > 22:  # Outside 6 AM - 10 PM
                events.append(SecurityEvent(
                    ...
                    ip_address=self._get_client_ip(request),
                    user_agent=request.headers.get('user-agent'),
                    details={'hour': hour, 'normal_hours': user_activity['normal_hours']},
    ...
    def _get_user_login_history(self, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _get_user_activity_pattern(self, user_id: str) -> Optional[Dict[str, Any]]:
...
class SecurityManager:
    ...
    def authenticate_request(self, request: Request, user: User) -> bool:
        ...
        allowed, rate_info = self.rate_limiter.check_rate_limit(
            rate_limit_key, 100, 3600, request  # 100 requests per hour
        )
