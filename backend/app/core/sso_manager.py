...
from app.models.domain_groups import DomainGroup, AccessLevel
...
class LDAPProvider(SSOProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.attributes = config.get("attributes", ["cn", "mail", "displayName", "memberOf"])
    ...
    async def authenticate(self, credentials: Dict[str, Any], db: Session) -> Tuple[User, Dict[str, Any]]:
        ...
        try:
            ...
            server = Server(self.server_url, use_ssl=self.use_ssl, connect_timeout=self.timeout)
            ...
            display_name = user_attrs.get("displayName", [""])[0] if user_attrs.get("displayName") else ""
        ...
        try:
            ...
            for ldap_group in ldap_groups:
                if ldap_group in self.group_mapping:
                    ...
                    domain_group = await self._create_domain_group_from_ldap(ldap_group, db)
                    if domain_group:
    ...
    async def _get_or_create_user(self, username: str, email: str, display_name: str, db: Session) -> User:
    ...
    async def _create_domain_group_from_ldap(self, ldap_group: str, db: Session) -> Optional[DomainGroup]:
        """Create domain group from LDAP group."""
        try:
            # Check if domain group already exists
            existing_group = db.query(DomainGroup).filter(
                DomainGroup.external_id == f"ldap:{ldap_group}"
            ).first()
...
class SAMLProvider(SSOProvider):
    ...
    async def authenticate(self, credentials: Dict[str, Any], db: Session) -> Tuple[User, Dict[str, Any]]:
        ...
        try:
            ...
            # Get or create user
            user = await self._get_or_create_user(username, email, first_name, last_name, db)
    ...
    async def sync_user_groups(self, user: User, groups: List[str], db: Session) -> List[str]:
    ...
    async def _get_or_create_user(self, username: str, email: str, first_name: str, last_name: str, db: Session) -> User:
    ...
    async def _create_domain_group_from_saml(self, saml_group: str, db: Session) -> Optional[DomainGroup]:
        """Create domain group from SAML group."""
        try:
            # Check if domain group already exists
            existing_group = db.query(DomainGroup).filter(
                DomainGroup.external_id == f"saml:{saml_group}"
            ).first()
...
class OAuthProvider(SSOProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all([self.client_id, self.client_secret, self.authorization_url, self.token_url]):
    ...
    async def _exchange_code_for_token(self, authorization_code: str, redirect_uri: str) -> Dict[str, Any]:
        """Exchange authorization code for access token."""
        token_data = {
    ...
    async def _create_domain_group_from_oauth(self, oauth_group: str, db: Session) -> Optional[DomainGroup]:
        """Create domain group from OAuth group."""
        try:
            # Check if domain group already exists
            existing_group = db.query(DomainGroup).filter(
                DomainGroup.external_id == f"oauth:{oauth_group}"
            ).first()
...
class SSOManager:
    ...
    def _init_providers(self):
        ...
        for provider_name, provider_config in providers_config.items():
            try:
                ...
                logger.error(f"Failed to initialize SSO provider {provider_name}: {str(e)}")
    
    async def authenticate(self, provider_name: str, credentials: Dict[str, Any], db: Session) -> Tuple[User, Dict[str, Any]]:
    ...
    async def get_user_info(self, provider_name: str, user_id: str, db: Session) -> Dict[str, Any]:
    ...
    async def sync_user_groups(self, provider_name: str, user: User, db: Session) -> List[str]:
