...
class AuditMetrics:
    ...
    async def record_audit_event(self, event_type: AuditEventType, duration_ms: int, success: bool = True) -> None:
        ...
        try:
            ...
            if current_avg:
                ...
                count = int(self.redis_client.get(f"{self.performance_prefix}count:{event_type.value}") or 0)
                new_avg = ((current_avg * count) + duration_ms) / (count + 1)
                self.redis_client.set(avg_key, new_avg)
                self.redis_client.incr(f"{self.performance_prefix}count:{event_type.value}")
            else:
                self.redis_client.set(avg_key, duration_ms)
                self.redis_client.set(f"{self.performance_prefix}count:{event_type.value}", 1)
            
        except Exception as e:
            logger.error(f"Failed to record audit metrics: {str(e)}")
    
    async def record_query_performance(self, query_type: str, duration_ms: int, result_count: int) -> None:
    ...
    async def record_cache_performance(self, cache_operation: str, hit: bool, duration_ms: int) -> None:
    ...
    async def get_performance_metrics(
        ...
        try:
            ...
            for event_type in event_types or list(AuditEventType):
                event_metrics = await self._get_event_metrics(event_type, start_date, end_date)
                metrics["event_metrics"][event_type.value] = event_metrics
            
            # Get performance metrics
            metrics["performance_metrics"] = await self._get_performance_metrics(start_date, end_date)
            
            # Get cache metrics
            metrics["cache_metrics"] = await self._get_cache_metrics(start_date, end_date)
            
            # Get database metrics
            metrics["database_metrics"] = await self._get_database_metrics(start_date, end_date)
    ...
    async def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for event_type in AuditEventType:
                ...
                total = int(self.redis_client.hget(key, f"total_{event_type.value}") or 0)
                success = int(self.redis_client.hget(key, f"success_{event_type.value}") or 0)
                failed = int(self.redis_client.hget(key, f"failed_{event_type.value}") or 0)
            ...
            for event_type in AuditEventType:
                ...
                if avg_duration:
                    metrics["rolling_averages"][event_type.value] = {
                        ...
                        "total_count": int(self.redis_client.get(f"{self.performance_prefix}count:{event_type.value}") or 0),
                    }
                ...
                if avg_duration:
                    ...
                    if avg_duration > 1000:
                        alerts.append({
                            ...
                            "threshold": 1000,
                            "severity": "warning" if avg_duration < 2000 else "critical",
                        })
    ...
    # Private methods
    async def _get_event_metrics(self, event_type: AuditEventType, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_events = query.filter(ExtendedAuditLog.action_result == "success").count()
            failed_events = total_events - successful_events
            
            # Calculate average duration
            duration_result = self.db.query(func.avg(ExtendedAuditLog.event_duration)).filter(
                ExtendedAuditLog.event_type == event_type,
                ExtendedAuditLog.timestamp >= start_date,
                ExtendedAuditLog.timestamp <= end_date,
            ...
            metrics.update({
                ...
                "avg_duration_ms": avg_duration,
                "peak_hour": peak_hour_query.hour.isoformat() if peak_hour_query else None,
    ...
    async def _get_cache_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    async def _get_database_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    async def _get_overview_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for event_type in AuditEventType:
                ...
                total = int(self.metrics.redis_client.hget(key, f"total_{event_type.value}") or 0)
                failed = int(self.metrics.redis_client.hget(key, f"failed_{event_type.value}") or 0)
            ...
            return {
                ...
                "error_rate_current_hour": (total_errors / total_events * 100) if total_events > 0 else 0,
                "system_status": "healthy" if total_errors / total_events < 0.1 else "degraded" if total_events > 0 else "unknown",
            }
    ...
    async def _get_performance_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for event_type in AuditEventType:
                ...
                if avg_duration:
                    performance["avg_response_times"][event_type.value] = float(avg_duration)
        ...
        try:
            ...
            for hour in range(24):
                ...
                trends["error_rate_trend"].append({
                    "timestamp": timestamp.isoformat(),
                    "value": (total_errors / total_events * 100) if total_events > 0 else 0,
                })
