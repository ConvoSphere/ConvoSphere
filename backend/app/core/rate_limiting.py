...
class RateLimiter:
    ...
    async def is_rate_limited(
        ...
        try:
            ...
            rate_key = (
                f"rate_limit:{key}:{identifier}" if identifier else f"rate_limit:{key}"
            )
    ...
    async def get_rate_limit_info(
        self, key: str, identifier: str | None = None
    ) -> dict:
        ...
        async def wrapper(*args, request: Request, **kwargs):
            try:
                ...
                is_limited, rate_info = await limiter.is_rate_limited(
                    key_prefix, max_requests, window_seconds, identifier
                )
...
RATE_LIMITS = {
    "upload": {"max_requests": 10, "window_seconds": 60, "key_prefix": "upload"},
    "search": {"max_requests": 100, "window_seconds": 60, "key_prefix": "search"},
    "chat": {"max_requests": 50, "window_seconds": 60, "key_prefix": "chat"},
    "api": {"max_requests": 1000, "window_seconds": 60, "key_prefix": "api"},
    "auth": {
        "max_requests": 5,
