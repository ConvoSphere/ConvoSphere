...
class CursorPagination:
    ...
    async def paginate_with_cursor(
        ...
        # Apply sorting
        sort_column = getattr(query.column_descriptions[0]['type'], pagination_params.sort_by)
        if pagination_params.sort_order == "desc":
...
class KeysetPagination:
    ...
    async def paginate_with_keyset(
        ...
        if keyset_values and len(keyset_values) == len(keyset_fields):
            keyset_conditions = []
            for field, value in zip(keyset_fields, keyset_values):
    ...
    def _generate_keyset_cursors(self, items: List[Any], keyset_fields: List[str]) -> Dict[str, str]:
...
class PaginationOptimizer:
    ...
    def choose_pagination_strategy(
        ...
        if pagination_params.sort_by != "id" and pagination_params.sort_by != "created_at":
            return PaginationType.KEYSET
        
        # Use offset pagination for small datasets with total count needed
        if pagination_params.include_total and (not estimated_total or estimated_total < 1000):
    ...
    async def paginate(
        ...
        query: Select,
        pagination_params: PaginationParams,
        strategy: Optional[PaginationType] = None,
        **kwargs
    ) -> PaginatedResult:
        """
        ...
        elif strategy == PaginationType.KEYSET:
