...
def init_sso_manager(config: Dict[str, Any] = None) -> SSOManager:
    ...
    logger.info(f"Global SSO Manager initialized with {len(config.get('providers', {}))} providers")
...
async def authenticate_user(provider_name: str, credentials: Dict[str, Any], db) -> tuple[Any, Dict[str, Any]]:
    """
