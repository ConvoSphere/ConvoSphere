...
class SSOManager:
    ...
    def _init_providers(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized {len(self.providers)} SSO providers: {list(self.providers.keys())}"
            )
    ...
    async def authenticate(
        ...
        if provider_name not in self.providers:
            raise SSOConfigurationError(
                f"SSO provider '{provider_name}' not configured"
            )
        ...
        try:
            ...
            logger.exception(
                f"Authentication failed for provider '{provider_name}': {str(e)}"
            )
    ...
    async def get_user_info(
        ...
        try:
            ...
            logger.exception(
                f"Failed to get user info from provider '{provider_name}': {str(e)}"
            )
    ...
    async def sync_user_groups(
        ...
        try:
            ...
            logger.exception(
                f"Failed to sync groups from provider '{provider_name}': {str(e)}"
            )
            return []
    ...
    async def validate_token(self, provider_name: str, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(
                f"Token validation failed for provider '{provider_name}': {str(e)}"
            )
    ...
    def is_provider_available(self, provider_name: str) -> bool:
        ...
        return (
            ...
            and self.providers[provider_name].is_enabled()
        )
    ...
    def get_enabled_providers(self) -> list[str]:
        ...
        """
        return [
            name for name, provider in self.providers.items() if provider.is_enabled()
        ]
