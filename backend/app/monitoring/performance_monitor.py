    ...
    def get_metrics(
        ...
        if tags:
            for key, value in tags.items():
                filtered_metrics = [
                    m for m in filtered_metrics if m.tags.get(key) == value
                ]
...
class AlertManager:
    ...
    def check_alerts(self, metrics_collector: MetricsCollector):
        ...
        for rule_name, rule in self.alert_rules.items():
            metrics = metrics_collector.get_metrics(
                name=rule["metric_name"], tags=rule["tags"]
            )
    ...
    def get_alerts(
        self, severity: AlertSeverity = None, since: datetime = None
    ) -> list[Alert]:
...
class PerformanceMonitor:
    ...
    def _setup_default_alerts(self):
        ...
        self.alert_manager.add_alert_rule(
            "high_cpu_usage", "cpu_percent", 80.0, AlertSeverity.WARNING, "gt"
        )

        # High memory usage
        self.alert_manager.add_alert_rule(
            "high_memory_usage", "memory_percent", 85.0, AlertSeverity.WARNING, "gt"
        )

        # High disk usage
        self.alert_manager.add_alert_rule(
            "high_disk_usage", "disk_percent", 90.0, AlertSeverity.WARNING, "gt"
        )
        ...
        # High error rate
        self.alert_manager.add_alert_rule(
            "high_error_rate", "http_errors_total", 10.0, AlertSeverity.ERROR, "gt"
        )
    ...
    def get_performance_report(self, since: datetime = None) -> dict[str, Any]:
        ...
        report = {
            ...
            "cache_metrics": {},
            "alerts": [
                alert.__dict__ for alert in self.alert_manager.get_alerts(since=since)
            ],
        }
        ...
        for name, metric_list in metrics_by_name.items():
            ...
            if name.startswith(("cpu_", "memory_", "disk_")):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            monitor.metrics_collector.record_timer(
                f"function_{func.__name__}_duration", duration
            )
