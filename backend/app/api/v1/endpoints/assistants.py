...
class AssistantCreate(BaseModel):
    ...
    personality: str | None = Field(None, description="Personality profile")
...
class AssistantUpdate(BaseModel):
    ...
    temperature: str | None = Field(None, description="Model temperature")
    max_tokens: str | None = Field(None, description="Maximum tokens")
    category: str | None = Field(None, description="Assistant category")
    ...
    is_template: bool | None = Field(
        None, description="Whether assistant is a template",
    )
    status: str | None = Field(None, description="Assistant status")
...
class ToolAssignmentRequest(BaseModel):
    ...
    config: dict[str, Any] | None = Field(None, description="Tool configuration")
...
async def update_assistant(
    ...
    try:
        ...
        assistant = assistant_service.update_assistant(
            assistant_id, current_user_id, **update_data,
        )
...
async def deactivate_assistant(
    ...
    try:
        assistant_service = AssistantService(db)
        assistant = assistant_service.deactivate_assistant(
            assistant_id, current_user_id,
        )
...
async def add_tool_to_assistant(
    ...
    try:
        ...
        logger.info(
            f"Added tool {tool_data.tool_id} to assistant {assistant_id} by user {current_user_id}",
        )
        return AssistantResponse(**assistant_dict)
...
async def remove_tool_from_assistant(
    ...
    try:
        ...
        logger.info(
            f"Removed tool {tool_id} from assistant {assistant_id} by user {current_user_id}",
        )
        return AssistantResponse(**assistant_dict)
