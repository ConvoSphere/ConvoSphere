...
class ConnectionManager:
    ...
    async def send_personal_message(self, message: str, user_id: str, conversation_id: str):
        """Send a message to a specific user in a conversation."""
        connection_id = f"{user_id}_{conversation_id}"
        if connection_id in self.active_connections:
            await self.active_connections[connection_id].send_text(message)
    
    async def broadcast_to_conversation(self, message: str, conversation_id: str, exclude_user: Optional[str] = None):
...
async def websocket_endpoint(
    ...
    try:
        ...
        if not conversation:
            await websocket.close(code=4002, reason="Conversation not found or access denied")
            return
            ...
            while True:
                ...
                if message_type == "message":
                    ...
                    try:
                        ...
                        messages = conversation_service.get_messages(conversation_id, limit=10)
                        ...
                        if ai_response:
                            ...
                            await manager.broadcast_to_conversation(ai_broadcast, conversation_id)
                        ...
                        await manager.send_personal_message(error_message, str(user.id), conversation_id)
                
                elif message_type == "typing":
                    # Handle typing indicator
                    is_typing = message_data.get("data", {}).get("is_typing", False)
                    await manager.send_typing_indicator(conversation_id, str(user.id), is_typing)
                    ...
                    await manager.send_personal_message(pong_message, str(user.id), conversation_id)
