...
async def advanced_search(
    ...
    filters: list[dict[str, Any]] | None = Body(None),
    facets: list[dict[str, Any]] | None = Body(None),
...
async def get_search_suggestions(
    ...
    try:
        ...
        suggestions = await search_service.get_search_suggestions(
            query=query, user_id=str(current_user.id), limit=limit
        )
...
async def get_trending_searches(
    ...
    try:
        ...
        return analytics.get("popular_queries", [])[:limit]
...
async def autocomplete_search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get autocomplete: {str(e)}"
        )
