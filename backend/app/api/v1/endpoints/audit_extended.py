...
async def get_audit_logs(
    ...
    data_classification: Optional[DataClassification] = Query(None, description="Filter by data classification"),
...
async def get_audit_log(
    ...
    try:
        ...
        if current_user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]:
            ...
            audit_log = db.query(ExtendedAuditLog).filter(
                ExtendedAuditLog.id == log_id,
                ExtendedAuditLog.user_id == str(current_user.id)
            ).first()
        else:
            from app.models.audit_extended import ExtendedAuditLog
            audit_log = db.query(ExtendedAuditLog).filter(ExtendedAuditLog.id == log_id).first()
        ...
        return {
            ...
            "data_classification": audit_log.data_classification.value if audit_log.data_classification else None,
...
@router.post("/policies", response_model=AuditPolicyResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/retention-rules", response_model=RetentionRuleResponse, status_code=status.HTTP_201_CREATED)
...
async def get_retention_rule(
    ...
    try:
        ...
        rule = db.query(AuditRetentionRule).filter(AuditRetentionRule.id == rule_id).first()
...
@router.delete("/compliance-reports/{report_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def audit_health_check(
    ...
    try:
        ...
        recent_logs = db.query(ExtendedAuditLog).filter(
            ExtendedAuditLog.timestamp >= datetime.now() - timedelta(hours=1)
        ).count()
