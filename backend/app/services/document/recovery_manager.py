...
from backend.app.models.knowledge import Document, DocumentStatus, DocumentProcessingJob
...
class DocumentRecoveryManager:
    ...
    async def execute_with_recovery(
        ...
        if recovery_strategies is None:
            recovery_strategies = [RecoveryStrategy.RETRY, RecoveryStrategy.ROLLBACK, RecoveryStrategy.SKIP]
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning(f"Operation {operation_id} failed (attempt {attempt + 1}): {e}")
                
                # Try recovery strategies
                if attempt < max_retries:
                    recovery_success = await self._attempt_recovery(
                        operation_id, document_id, operation, rollback_operation,
                        recovery_strategies, e
                    )
                    if recovery_success:
                    ...
                    await self._update_document_status(document_id, DocumentStatus.ERROR, str(e))
                    logger.error(f"Operation {operation_id} failed after {max_retries} retries")
                    return False
    ...
    async def _attempt_recovery(
        ...
        document_id: str,
        operation: Callable,
        rollback_operation: Optional[Callable],
        ...
        for strategy in strategies:
            ...
            try:
                if strategy == RecoveryStrategy.RETRY:
                    ...
                    success = await self._rollback_operation(rollback_operation, document_id)
                ...
                if success:
                    ...
                    recovery_attempt.error_message = f"Recovery strategy {strategy.value} failed"
    ...
    async def _rollback_operation(self, rollback_operation: Optional[Callable], document_id: str) -> bool:
    ...
    async def _fallback_operation(self, document_id: str) -> bool:
        ...
        try:
            ...
            from backend.app.services.document.processors.text_processor import TextProcessor
    ...
    def get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        for strategy in RecoveryStrategy:
            ...
            strategy_stats[strategy.value] = {
                ...
                "success_rate": strategy_successes / len(strategy_attempts) if strategy_attempts else 0
            }
...
class DocumentStateManager:
    ...
    def create_rollback_point(self, document_id: str) -> Optional[str]:
        ...
        try:
            ...
            document.processing_options["rollback_points"] = document.processing_options.get("rollback_points", {})
...
# Global instances
_recovery_manager: Optional[DocumentRecoveryManager] = None
_state_manager: Optional[DocumentStateManager] = None

def get_recovery_manager(db: Session, error_handler: DocumentErrorHandler) -> DocumentRecoveryManager:
