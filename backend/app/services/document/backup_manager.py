...
class DocumentBackupManager:
        ...
        for document in documents:
            try:
                ...
                if backup_type == BackupType.FULL and document.file_path:
                    file_backup_path = backup_path / "files" / f"{document.id}_{document.file_name}"
    ...
    async def _restore_documents(self, backup_path: Path, document_ids: Optional[List[str]]):
        ...
        for doc_file in documents_path.glob("*.json"):
            try:
                ...
                existing_doc = self.db.query(Document).filter(
                    Document.id == document_backup.document_id
                ).first()
                
                if existing_doc:
                    # Update existing document
                    for key, value in document_backup.document_data.items():
                        if hasattr(existing_doc, key) and key not in ['id', 'created_at']:
                ...
                if document_backup.file_path and os.path.exists(document_backup.file_path):
    ...
    async def _restore_chunks(self, document_backup: DocumentBackup):
        ...
        try:
            ...
            for chunk_data in document_backup.chunks_data:
                ...
                chunk_data['updated_at'] = datetime.fromisoformat(chunk_data['updated_at'])
                if chunk_data['embedding_created_at']:
                    chunk_data['embedding_created_at'] = datetime.fromisoformat(chunk_data['embedding_created_at'])
            ...
            logger.error(f"Failed to restore chunks for document {document_backup.document_id}: {e}")
    
    async def _restore_processing_jobs(self, backup_path: Path, document_ids: Optional[List[str]]):
        ...
        try:
            ...
            for job_data in jobs_data:
                ...
                # Check if job exists
                existing_job = self.db.query(DocumentProcessingJob).filter(
                    DocumentProcessingJob.id == job_data['id']
                ).first()
    ...
    def cleanup_expired_backups(self) -> int:
        ...
        for backup_id, metadata in list(self.backup_metadata.items()):
            ...
            expiration_date = metadata.created_at + timedelta(days=metadata.retention_days)
    ...
    def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        successful_backups = sum(1 for m in self.backup_metadata.values() if m.status == BackupStatus.COMPLETED)
        failed_backups = sum(1 for m in self.backup_metadata.values() if m.status == BackupStatus.FAILED)
...
_backup_manager: Optional[DocumentBackupManager] = None
