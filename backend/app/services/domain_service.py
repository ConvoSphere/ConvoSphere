...
class DomainService:
    ...
    def create_domain_group(
        ...
        if not self._can_create_domain_group(current_user):
            raise PermissionDeniedError("Insufficient permissions to create domain groups")
        
        # Validate parent domain if specified
        if domain_data.parent_domain_id:
            parent_domain = self.get_domain_group_by_id(domain_data.parent_domain_id, current_user)
            ...
            if not self._can_manage_domain_group(parent_domain, current_user):
                raise PermissionDeniedError("Cannot create domain group under this parent")
    ...
    def list_domain_groups(
        ...
        if search_params.organization_id:
            query = query.filter(DomainGroup.organization_id == search_params.organization_id)
        elif current_user.organization_id:
            # Filter by user's organization if not super admin
            if current_user.role not in [UserRole.SUPER_ADMIN]:
                query = query.filter(DomainGroup.organization_id == current_user.organization_id)
        
        if search_params.parent_domain_id:
            query = query.filter(DomainGroup.parent_domain_id == search_params.parent_domain_id)
    ...
    def delete_domain_group(
        ...
        child_domains = self.db.query(DomainGroup).filter(
            DomainGroup.parent_domain_id == domain_id
        ).count()
    ...
    def accept_domain_invitation(
        ...
        # Check if user is already a member
        domain_group = self.get_domain_group_by_id(str(invitation.domain_group_id), current_user)
    ...
    def get_domain_stats(
        ...
        # Apply access control
        if current_user and current_user.role not in [UserRole.SUPER_ADMIN, UserRole.ADMIN]:
            query = query.filter(
                or_(
                    DomainGroup.is_public == True,
                    DomainGroup.members.any(id=current_user.id),
    ...
    def _add_domain_member(
        ...
        domain_group = self.db.query(DomainGroup).filter(DomainGroup.id == domain_id).first()
    ...
    def _can_access_domain_group(self, domain_group: DomainGroup, user: User) -> bool:
        ...
        if user.role == UserRole.ADMIN and domain_group.organization_id == user.organization_id:
            return True
    ...
    def _can_manage_domain_resources(self, domain_group: DomainGroup, user: User) -> bool:
