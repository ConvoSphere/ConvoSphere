...
from app.services.tool_executor import tool_executor, ToolExecution
...
class AssistantEngine:
    ...
    async def _process_request(
        ...
        try:
            ...
            if ai_response.tool_calls:
                ...
                if tool_results:
                    follow_up_messages = await context_manager.get_messages_for_completion(
                        request.conversation_id,
                        request.user_id
                    )
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        completed = len([r for r in self.processing_results.values() if r.status == ProcessingStatus.COMPLETED])
        failed = len([r for r in self.processing_results.values() if r.status == ProcessingStatus.FAILED])
        pending = len([r for r in self.processing_results.values() if r.status == ProcessingStatus.PENDING])
        processing = len([r for r in self.processing_results.values() if r.status == ProcessingStatus.PROCESSING])
        
        avg_processing_time = 0
        if completed > 0:
            processing_times = [r.processing_time for r in self.processing_results.values() if r.processing_time]
            if processing_times:
