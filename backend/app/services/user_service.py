...
class UserService:
    ...
    def create_user(self, user_data: UserCreate, current_user: Optional[User] = None) -> User:
        """Create a new user."""
    ...
    def get_user_by_external_id(self, external_id: str, auth_provider: AuthProvider) -> Optional[User]:
        """Get user by external ID and auth provider."""
        return self.db.query(User).options(
            joinedload(User.groups)
        ).filter(
            and_(User.external_id == external_id, User.auth_provider == auth_provider)
        ).first()
    
    def update_user(self, user_id: str, user_data: UserUpdate, current_user: User) -> User:
        """Update user."""
    ...
    def list_users(self, search_params: UserSearchParams, current_user: User) -> UserListResponse:
        ...
        if search_params.group_id:
            query = query.join(User.groups).filter(UserGroup.id == search_params.group_id)
    ...
    def bulk_update_users(self, bulk_data: UserBulkUpdate, current_user: User) -> int:
        ...
        if not current_user.has_permission("user:write"):
            raise PermissionDeniedError("Insufficient permissions for bulk user operations")
    ...
    def create_sso_user(self, sso_data: SSOUserCreate) -> User:
        ...
        # Check if user already exists
        existing_user = self.get_user_by_external_id(sso_data.external_id, sso_data.auth_provider)
        if existing_user:
    ...
    def create_group(self, group_data: UserGroupCreate, current_user: User) -> UserGroup:
    ...
    def get_group_by_id(self, group_id: str) -> Optional[UserGroup]:
        """Get group by ID."""
        return self.db.query(UserGroup).options(
            joinedload(UserGroup.users)
        ).filter(UserGroup.id == group_id).first()
    
    def update_group(self, group_id: str, group_data: UserGroupUpdate, current_user: User) -> UserGroup:
        """Update user group."""
    ...
    def list_groups(self, organization_id: Optional[str] = None, current_user: User = None) -> List[UserGroup]:
        """List user groups."""
        ...
        if organization_id:
            ...
            query = query.filter(UserGroup.organization_id == current_user.organization_id)
    ...
    def assign_users_to_groups(self, assignment: UserGroupAssignment, current_user: User) -> int:
        """Assign multiple users to groups."""
        if not current_user.has_permission("user:write"):
            raise PermissionDeniedError("Insufficient permissions for group assignments")
        
        users = self.db.query(User).filter(User.id.in_(assignment.user_ids)).all()
        groups = self.db.query(UserGroup).filter(UserGroup.id.in_(assignment.group_ids)).all()
    ...
    # User statistics
    def get_user_stats(self, organization_id: Optional[str] = None, current_user: User = None) -> UserStats:
        """Get user statistics."""
