...
class PerformanceMonitor:
    ...
    def _check_metric_alerts(self, metric: Metric):
        ...
        if "max" in threshold and current_value > threshold["max"]:
            self._create_alert(
                ...
                level=AlertLevel.WARNING if threshold.get("level") == "warning" else AlertLevel.ERROR,
    ...
    def _calculate_request_rate(self) -> float:
        ...
        recent_requests = [req for req in self.request_times if req["timestamp"] > cutoff_time]
    ...
    def _calculate_error_rate(self) -> float:
        ...
        # Calculate rate over last minute
        cutoff_time = datetime.now() - timedelta(minutes=1)
        recent_errors = [error for error in self.error_counts if error["timestamp"] > cutoff_time]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        trend_snapshots = [
            snap for snap in self.performance_snapshots
