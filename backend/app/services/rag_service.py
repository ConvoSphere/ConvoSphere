...
from app.schemas.rag import (
    RAGConfig, RAGRequest, RAGResponse, RAGResult, RAGContext, RAGMetrics,
...
class RAGService:
    ...
    async def _adaptive_retrieval(
        ...
        if query_analysis['is_technical']:
            ...
            return await self._semantic_retrieval(request, config, conversation_history)
        elif query_analysis['is_conversational']:
    ...
    async def _process_results(
        ...
        # Limit results
        rag_results = rag_results[:config.max_results]
    ...
    def _get_config(self, config_id: Optional[str]) -> Optional[RAGConfig]:
    ...
    def _get_sources_queried(self, results: List[RAGResult]) -> List[str]:
        ...
        return list(set(result.source for result in results))
    ...
    def _build_contextual_query(self, query: str, history: List[Dict[str, Any]]) -> str:
        ...
        contextual_query = f"{query} {' '.join(set(context_terms))}"
