...
from app.core.exceptions import AIError, ConfigurationError, ValidationError as AppValidationError
from app.schemas.conversation_intelligence import (
    ConversationSummary, TopicInfo, SentimentAnalysis, ConversationAnalytics,
    ConversationIntelligenceRequest, ConversationIntelligenceResponse,
    IntelligenceMetrics, SummaryRequest, TopicDetectionRequest, SentimentAnalysisRequest,
    SentimentType, TopicCategory, SummaryType
)
...
class ConversationIntelligenceService:
    ...
    async def _generate_analytics(
        ...
        total_participants = len(set(msg.get('user_id') for msg in messages if msg.get('user_id')))
        duration_minutes = self._calculate_duration(conversation)
    ...
    async def _call_ai_model(self, model_type: str, prompt: str, max_length: Optional[int] = None) -> str:
        ...
        elif model_type == 'topic_detection':
    ...
    def _extract_participants(self, conversation: Dict[str, Any]) -> List[str]:
        ...
        messages = conversation.get('messages', [])
        return list(set(msg.get('user_id') for msg in messages if msg.get('user_id')))
    ...
    async def _analyze_participant_sentiment(self, messages: List[Dict[str, Any]]) -> Dict[str, SentimentType]:
        ...
        return {participant: SentimentType.POSITIVE for participant in participants}
    ...
    def _calculate_participant_engagement(self, messages: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        participants = set(msg.get('user_id') for msg in messages if msg.get('user_id'))
        return {participant: 0.8 for participant in participants}
    ...
    def _calculate_quality_score(self, messages: List[Dict[str, Any]]) -> float:
        ...
        participant_variety = len(set(msg.get('user_id') for msg in messages))
    ...
    def _calculate_engagement_score(self, messages: List[Dict[str, Any]]) -> float:
        ...
        engagement_score = max(0.0, 1.0 - (avg_response_time / 300.0))  # 5 minutes max
    ...
    def _calculate_clarity_score(self, messages: List[Dict[str, Any]]) -> float:
        ...
        clarity_score = max(0.0, 1.0 - (variance / 10000.0))
    ...
    def _find_peak_activity_time(self, messages: List[Dict[str, Any]]) -> Optional[datetime]:
    ...
    async def _get_cached_result(self, cache_key: str) -> Optional[ConversationIntelligenceResponse]:
    ...
    def _update_metrics(
        ...
        response: Optional[ConversationIntelligenceResponse] = None,
