...
class ToolExecutor:
    ...
    def get_tool_schema(self, tool_id: str) -> Optional[Dict[str, Any]]:
        ...
        for param in tool_def.parameters:
            ...
            if param.min_value is not None:
                schema["function"]["parameters"]["properties"][param.name]["minimum"] = param.min_value
            
            if param.max_value is not None:
                schema["function"]["parameters"]["properties"][param.name]["maximum"] = param.max_value
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        completed = len([e for e in self.executions.values() if e.status == ToolExecutionStatus.COMPLETED])
        failed = len([e for e in self.executions.values() if e.status == ToolExecutionStatus.FAILED])
        timeout = len([e for e in self.executions.values() if e.status == ToolExecutionStatus.TIMEOUT])
        
        avg_execution_time = 0
        if completed > 0:
            execution_times = [e.execution_time for e in self.executions.values() if e.execution_time]
            if execution_times:
