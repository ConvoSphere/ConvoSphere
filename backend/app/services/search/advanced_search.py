...
class AdvancedSearchService:
        ...
        try:
            ...
            if search_type == SearchType.FULL_TEXT:
                results = await self._full_text_search(
                    parsed_query, applied_filters, user_id
                )
                ...
                results = await self._faceted_search(
                    parsed_query, applied_filters, user_id, facets
                )
            ...
            if include_highlights:
                paginated_results = await self._add_highlights(
                    paginated_results, parsed_query
                )
    ...
    async def _full_text_search(
        self, parsed_query: dict[str, Any], filters: dict[str, Any], user_id: str
    ) -> list[SearchResult]:
    ...
    async def _semantic_search(
        ...
        try:
            ...
            for weaviate_result in weaviate_results:
                chunk = (
                    self.db.query(DocumentChunk)
                    .filter(DocumentChunk.id == weaviate_result["chunk_id"])
                    .first()
                )
    ...
    async def _fuzzy_search(
        ...
        for term in parsed_query["terms"]:
            ...
            fuzzy_conditions.append(func.similarity(DocumentChunk.content, term) > 0.3)

        if fuzzy_conditions:
            query = (
                self.db.query(DocumentChunk)
                .join(Document)
                .filter(Document.user_id == user_id, or_(*fuzzy_conditions))
            )
    ...
    def _apply_sql_filters(self, query, filters: dict[str, Any]):
        ...
        for field, value in filters.items():
            if field == "document_type":
                ...
                query = query.filter(
                    and_(Document.year >= start_year, Document.year <= end_year)
                )
            elif field == "status":
    ...
    def _convert_filters_for_weaviate(
    ...
    def _calculate_text_score(
        self, chunk: DocumentChunk, parsed_query: dict[str, Any]
    ) -> float:
    ...
    def _calculate_fuzzy_score(
        ...
        score = 0.0
        for term in parsed_query["terms"]:
            similarity = self.db.query(func.similarity(chunk.content, term)).scalar()
            score += similarity
    ...
    def _sort_results(
        ...
        if sort_by == "relevance":
            ...
            results.sort(
                key=lambda x: x.document_info.get("created_at", ""), reverse=reverse
            )
        elif sort_by == "title":
            results.sort(
                key=lambda x: x.document_info.get("title", ""), reverse=reverse
            )
        elif sort_by == "author":
            results.sort(
                key=lambda x: x.document_info.get("author", ""), reverse=reverse
            )

        return results

    async def _add_highlights(
        self, results: list[SearchResult], parsed_query: dict[str, Any]
    ) -> list[SearchResult]:
    ...
    async def _compute_facets(
        self, results: list[SearchResult], facets: list[SearchFacet]
    ) -> dict[str, Any]:
    ...
    async def _log_search_query(
        self, query: str, user_id: str, search_type: SearchType
    ):
    ...
    async def get_search_suggestions(
        ...
        popular_terms = (
            self.db.query(
                SearchQuery.query, func.count(SearchQuery.id).label("frequency")
            )
            .filter(
                SearchQuery.user_id == user_id, SearchQuery.query.ilike(f"%{query}%")
            )
        ...
        for term, frequency in popular_terms:
            suggestions.append(
                SearchSuggestion(
                    term=term, frequency=frequency, score=frequency * 0.1, type="term"
                )
            )

        # Get tag suggestions
        tag_suggestions = (
            self.db.query(Tag).filter(Tag.name.ilike(f"%{query}%")).limit(limit).all()
        )
    ...
    async def get_search_analytics(
        ...
        total_searches = (
            ...
            .filter(SearchQuery.user_id == user_id, SearchQuery.timestamp >= start_date)
            .count()
        )
        ...
        search_types = (
            self.db.query(
                SearchQuery.search_type, func.count(SearchQuery.id).label("count")
            )
            .filter(SearchQuery.user_id == user_id, SearchQuery.timestamp >= start_date)
        ...
        return {
            ...
            "popular_queries": [
                {"query": pq.query, "count": pq.count} for pq in popular_queries
            ],
...
_search_service: AdvancedSearchService | None = None
