...
class WeaviateService:
    ...
    def index_message(self, conversation_id: str, message_id: str, content: str, role: str, metadata: Optional[Dict[str, Any]] = None):
    ...
    def semantic_search_messages(self, query: str, conversation_id: Optional[str] = None, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            ).with_near_text({"concepts": [query]}).with_where(filters).with_limit(limit).do()
            return result["data"]["Get"]["ChatMessage"]
        except Exception as e:
            logger.error(f"Semantic search failed: {e}")
            return []
    
    def index_knowledge(self, doc_id: str, content: str, source: str, metadata: Optional[Dict[str, Any]] = None):
