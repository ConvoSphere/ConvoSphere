...
class AssistantService:
        ...
        if include_public:
            query = query.filter(
                or_(
                    ...
                    and_(Assistant.is_public == True, Assistant.status == AssistantStatus.ACTIVE)
                )
            )
        else:
    ...
    def update_assistant(
        ...
        **kwargs
        ...
        if assistant.creator_id != user_id:
            logger.warning(f"Unauthorized update attempt: user {user_id} tried to update assistant {assistant_id}")
            return None
    ...
    def delete_assistant(self, assistant_id: str, user_id: str) -> bool:
        ...
        if assistant.creator_id != user_id:
            logger.warning(f"Unauthorized delete attempt: user {user_id} tried to delete assistant {assistant_id}")
            return False
    ...
    def activate_assistant(self, assistant_id: str, user_id: str) -> Optional[Assistant]:
        """
    ...
    def deactivate_assistant(self, assistant_id: str, user_id: str) -> Optional[Assistant]:
        """
