...
from app.models.knowledge import DocumentProcessingJob, Document, DocumentStatus
...
class BackgroundJobService:
    ...
    def _process_document_job(self, job: DocumentProcessingJob, db: Session):
        ...
        try:
            ...
            logger.error(f"Error in document processing job: {e}")
            raise
    ...
    def _reprocess_document_job(self, job: DocumentProcessingJob, db: Session):
        ...
        try:
            ...
            logger.error(f"Error in document reprocessing job: {e}")
            raise
        ...
        try:
            ...
            for i, file_info in enumerate(files):
                try:
                    ...
                    success = asyncio.run(knowledge_service.process_document(str(document.id)))
                    if not success:
                        logger.warning(f"Failed to process file {file_info.get('name')}")
            ...
            logger.error(f"Error in bulk import job: {e}")
            raise
