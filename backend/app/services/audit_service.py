...
class AuditContext:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        duration = int((time.time() - self.start_time) * 1000)  # Convert to milliseconds
...
class AuditService:
    ...
    async def log_event(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Audit event logged: {event_type} for user {user.username if user else 'anonymous'}")
            return audit_log
    ...
    def audit_context(
        ...
        **kwargs
        ...
        return AuditContext(self, event_type=event_type, event_category=event_category, user=user, request=request, **kwargs)
    ...
    async def get_audit_logs(
        ...
        try:
            ...
            if compliance_framework:
                query = query.filter(ExtendedAuditLog.compliance_frameworks.contains([compliance_framework.value]))
            
            if data_classification:
                query = query.filter(ExtendedAuditLog.data_classification == data_classification)
            ...
            # Apply pagination
            offset = (page - 1) * size
            audit_logs = query.order_by(desc(ExtendedAuditLog.timestamp)).offset(offset).limit(size).all()
            ...
            for log in audit_logs:
                log_data = {
                    ...
                    "data_classification": log.data_classification.value if log.data_classification else None,
    ...
    async def cleanup_expired_logs(self) -> int:
        ...
        try:
            ...
            retention_rules = self.db.query(AuditRetentionRule).filter(AuditRetentionRule.enabled == True).all()
            ...
            for rule in retention_rules:
                ...
                query = self.db.query(ExtendedAuditLog).filter(ExtendedAuditLog.timestamp < expiry_date)
                
                if rule.event_types:
                    query = query.filter(ExtendedAuditLog.event_type.in_(rule.event_types))
                
                if rule.event_categories:
                    query = query.filter(ExtendedAuditLog.event_category.in_(rule.event_categories))
                
                if rule.severity_levels:
                    query = query.filter(ExtendedAuditLog.severity.in_(rule.severity_levels))
                
                if rule.compliance_frameworks:
                    query = query.filter(ExtendedAuditLog.compliance_frameworks.overlap(rule.compliance_frameworks))
                
                if rule.data_classification:
                    query = query.filter(ExtendedAuditLog.data_classification == rule.data_classification)
    ...
    async def get_audit_statistics(
        ...
        try:
            ...
            if organization_id:
                query = query.filter(ExtendedAuditLog.organization_id == organization_id)
            ...
            for category in AuditEventCategory:
                count = query.filter(ExtendedAuditLog.event_category == category).count()
                if count > 0:
            ...
            ).group_by(ExtendedAuditLog.username).order_by(desc('event_count')).limit(10).all()
            
            # Compliance statistics
            compliance_stats = {}
            for framework in ComplianceFramework:
                count = query.filter(ExtendedAuditLog.compliance_frameworks.contains([framework.value])).count()
                if count > 0:
            ...
            # Failed actions
            failed_actions = query.filter(ExtendedAuditLog.action_result == "failure").count()
            ...
            return {
                ...
                "top_users": [{"username": user.username, "event_count": user.event_count} for user in top_users],
    ...
    # Private methods
    async def _should_log_event(self, event_type: AuditEventType, event_category: AuditEventCategory, user: Optional[User]) -> bool:
        """Check if event should be logged based on audit policies."""
        try:
            # Get applicable policies
            policies = self.db.query(AuditPolicy).filter(AuditPolicy.enabled == True).all()
            ...
            for policy in policies:
                ...
                if policy.event_categories and event_category.value not in policy.event_categories:
                    continue
    ...
    async def _apply_retention_rules(self, audit_log: ExtendedAuditLog) -> None:
        ...
        try:
            rules = self.db.query(AuditRetentionRule).filter(AuditRetentionRule.enabled == True).all()
            
            for rule in rules:
                # Check if rule applies
                if rule.event_types and audit_log.event_type.value not in rule.event_types:
                    continue
                
                if rule.event_categories and audit_log.event_category.value not in rule.event_categories:
                    continue
                
                if rule.severity_levels and audit_log.severity.value not in rule.severity_levels:
                    continue
                
                if rule.compliance_frameworks and audit_log.compliance_frameworks:
                    if not any(fw in rule.compliance_frameworks for fw in audit_log.compliance_frameworks):
                        continue
                
                if rule.data_classification and audit_log.data_classification != rule.data_classification:
                    continue
    ...
    async def _check_alerts(self, audit_log: ExtendedAuditLog) -> None:
        ...
        try:
            ...
            for alert in alerts:
                ...
                if alert.event_types and audit_log.event_type.value not in alert.event_types:
                    continue
                
                if alert.severity_levels and audit_log.severity.value not in alert.severity_levels:
                    continue
    ...
    async def _trigger_alert(self, alert: AuditAlert, audit_log: ExtendedAuditLog, count: int) -> None:
        ...
        try:
            ...
            logger.warning(f"Audit alert triggered: {alert.name} - {count} events in {alert.threshold_period}s")
    ...
    async def _analyze_compliance_findings(self, audit_logs: List[ExtendedAuditLog], framework: ComplianceFramework) -> List[Dict[str, Any]]:
        ...
        security_events = [log for log in audit_logs if log.event_category == AuditEventCategory.SECURITY]
    ...
    async def _generate_compliance_recommendations(self, findings: List[Dict[str, Any]], framework: ComplianceFramework) -> List[Dict[str, Any]]:
    ...
    async def _calculate_compliance_metrics(self, audit_logs: List[ExtendedAuditLog], framework: ComplianceFramework) -> Dict[str, Any]:
        """Calculate compliance metrics."""
        total_events = len(audit_logs)
        failed_events = len([log for log in audit_logs if log.action_result == "failure"])
        security_events = len([log for log in audit_logs if log.event_category == AuditEventCategory.SECURITY])
        ...
        return {
            ...
            "security_incident_rate": (security_events / total_events * 100) if total_events > 0 else 0,
        }
    
    async def _archive_logs(self, logs: List[ExtendedAuditLog], rule: AuditRetentionRule) -> None:
