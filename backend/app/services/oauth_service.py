...
class OAuthService:
    ...
    def _setup_oauth_clients(self):
        ...
        # Microsoft OAuth2
        if (
            self.settings.sso_microsoft_enabled
            and self.settings.sso_microsoft_client_id
        ):
            self.oauth.register(
                name="microsoft",
    ...
    async def handle_callback(self, provider: str, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            return {"user_info": user_info, "token": token, "provider": provider}
        except Exception as e:
    ...
    def _is_provider_enabled(self, provider: str) -> bool:
        ...
        if provider == "google":
            return (
                self.settings.sso_google_enabled
                and self.settings.sso_google_client_id
                and self.settings.sso_google_client_secret
            )
        if provider == "microsoft":
            return (
                self.settings.sso_microsoft_enabled
                and self.settings.sso_microsoft_client_id
                and self.settings.sso_microsoft_client_secret
            )
        if provider == "github":
            return (
                self.settings.sso_github_enabled
                and self.settings.sso_github_client_id
                and self.settings.sso_github_client_secret
            )
        return False
