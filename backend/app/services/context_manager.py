...
class ContextManager:
    ...
    async def add_knowledge_context(
        ...
        for chunk in knowledge_chunks:
            ...
            if context.current_tokens + token_count > context.max_tokens:
                ...
                if context.current_tokens + token_count > context.max_tokens:
                    logger.warning(f"Cannot add knowledge context - limit exceeded")
                    return False
    ...
    async def add_tool_result(
        ...
        if context.current_tokens + token_count > context.max_tokens:
            ...
            if context.current_tokens + token_count > context.max_tokens:
                logger.warning(f"Cannot add tool result - limit exceeded")
                return False
    ...
    async def get_context_summary(
        ...
        return {
            ...
            "items_by_type": {
                item_type.value: len([item for item in context.items if item.type == item_type])
    ...
    async def _optimize_context(self, context: ConversationContext):
        ...
        def sort_key(item: ContextItem) -> Tuple[float, datetime]:
            ...
            recency_score = (datetime.now() - item.timestamp).total_seconds() / 3600  # Hours ago
        ...
        tokens_to_remove = context.current_tokens - context.max_tokens + self.token_buffer
        ...
        for item in sorted_items:
            ...
            if item.type == ContextType.SYSTEM and removed_tokens < tokens_to_remove * 0.8:
                continue
        ...
        logger.info(f"Optimized context: removed {len(items_to_remove)} items, "
