...
const testEndpoint = async (endpoint) => {
  ...
  try {
    ...
    if (success) {
      ...
      log(
        `✅ ${endpoint.name}: ${status} (${formatResponseTime(responseTime)})`,
        "success",
      );
    } else {
      testResults.failed++;
      ...
      log(
        `${color}❌ ${endpoint.name}: ${status} - ${error.error}\x1b[0m`,
        "error",
      );
    }
...
const generateSummary = () => {
  ...
  const avgResponseTime =
    testResults.responseTimes.length > 0
  ...
  if (successRate >= 90) {
    ...
    log(
      "\n⚠️  Backend integration has some issues. Check failed tests.",
      "warning",
    );
  } else {
    log(
      "\n🚨 Backend integration has significant issues. Review failed tests.",
      "error",
    );
  }
};
...
const analyzePerformance = () => {
  ...
  if (p99 > 5000) {
    log(
      "🚨 Some endpoints are very slow (>5s). Immediate attention needed.",
      "error",
    );
  }
};
