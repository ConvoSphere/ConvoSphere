...
const UserBehaviorAnalysis: React.FC<UserBehaviorAnalysisProps> = ({
    ...
    return (
      <ModernCard>
        <div style={{ textAlign: "center", padding: "40px" }}>
          <Text type="secondary">
            {t("intelligence.no_user_behavior_data")}
          </Text>
        </div>
      </ModernCard>
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: UserBehaviorMetrics, b: UserBehaviorMetrics) =>
      ...
      render: (value: number) => (
        <Space direction="vertical" size="small">
          ...
          <Progress
            ...
            size="small"
            status={
              value >= 4 ? "success" : value >= 3 ? "normal" : "exception"
            }
            showInfo={false}
          />
        </Space>
    ...
    },
    {
      ...
      render: (topics: string[]) => (
        <Space wrap>
          ...
          {topics.length > 3 && <Tag size="small">+{topics.length - 3}</Tag>}
        </Space>
      ),
    },
  ];
        ...
        <Col xs={24} sm={12} lg={6}>
          <ModernCard>
            <div style={{ textAlign: "center" }}>
              ...
              <Text strong style={{ fontSize: "24px" }}>
                {(
                  data.reduce((sum, user) => sum + user.totalConversations, 0) /
                  data.length
                ).toFixed(1)}
              </Text>
            </div>
        ...
        <Col xs={24} sm={12} lg={6}>
          <ModernCard>
            <div style={{ textAlign: "center" }}>
              ...
              <Text strong style={{ fontSize: "24px" }}>
                {(
                  data.reduce((sum, user) => sum + user.satisfactionScore, 0) /
                  data.length
                ).toFixed(1)}
              </Text>
            </div>
        ...
        <Col xs={24} sm={12} lg={6}>
          <ModernCard>
            <div style={{ textAlign: "center" }}>
              ...
              <Text strong style={{ fontSize: "24px" }}>
                {(
                  data.reduce((sum, user) => sum + user.avgResponseTime, 0) /
                  data.length
                ).toFixed(1)}
                s
              </Text>
              ...
              <Card size="small">
                <div style={{ textAlign: "center" }}>
                  ...
                  <Progress
                    percent={
                      (user.totalConversations /
                        sortedUsers[0].totalConversations) *
                      100
                    }
                    size="small"
                    showInfo={false}
      ...
      <ModernCard title={t("intelligence.topic_preferences_summary")}>
        <Row gutter={[16, 16]}>
          {Array.from(new Set(data.flatMap((user) => user.preferredTopics)))
            .slice(0, 6)
            .map((topic) => {
              const usersWithTopic = data.filter((user) =>
                  ...
                  <Card size="small">
                    <div style={{ textAlign: "center" }}>
                      ...
                      <div style={{ marginTop: 4 }}>
                        <Text type="secondary">
                          {(
                            (usersWithTopic.length / data.length) *
                            100
                          ).toFixed(1)}
                          % {t("intelligence.of_users")}
                        </Text>
                      </div>
                    </div>
                  </Card>
