...
import { Row, Col, Card, Progress, Typography, Space, Tag, List, Avatar } from 'antd';
import { 
  SmileOutlined, 
  MehOutlined, 
  FrownOutlined,
...
const SentimentAnalysis: React.FC<SentimentAnalysisProps> = ({ 
  ...
  const sentimentDistribution = [
    { type: t('intelligence.positive'), value: data.messageSentiments.filter(m => m.sentiment === 'positive').length },
    { type: t('intelligence.neutral'), value: data.messageSentiments.filter(m => m.sentiment === 'neutral').length },
    { type: t('intelligence.negative'), value: data.messageSentiments.filter(m => m.sentiment === 'negative').length },
  ];
          ...
          <ModernCard>
            <div style={{ textAlign: 'center' }}>
              ...
              <Progress
                ...
                status={data.sentimentScore > 0 ? 'success' : data.sentimentScore < 0 ? 'exception' : 'normal'}
                style={{ marginTop: 16 }}
              />
            </div>
          </ModernCard>
        ...
        <Col xs={24} md={8}>
          <ModernCard>
            <div style={{ textAlign: 'center' }}>
              ...
              <Text strong style={{ fontSize: '24px', color: getSentimentColor(data.overallSentiment) }}>
        ...
        <List
          ...
          renderItem={(message, index) => (
            <List.Item>
              <List.Item.Meta
                avatar={
                  <Avatar icon={getSentimentIcon(message.sentiment)} />
                }
                title={
                  <Space>
                    <Text strong>{t('intelligence.message')} {index + 1}</Text>
                ...
                description={
                  <Space direction="vertical" size="small">
                    <Text>
                      {t('intelligence.sentiment_score')}: {message.score.toFixed(3)}
                    </Text>
                    <Progress
                      percent={Math.abs(message.score) * 100}
                      size="small"
                      status={message.score > 0 ? 'success' : message.score < 0 ? 'exception' : 'normal'}
                      showInfo={false}
                    />
                  </Space>
                }
