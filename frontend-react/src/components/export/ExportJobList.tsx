          ...
          expandedRowRender: (record) => (
            <div style={{ padding: '16px' }}>
              ...
              <div style={{ marginBottom: '8px' }}>
                <strong>{t('export.format')}:</strong> {record.options.format.toUpperCase()}
              </div>
              {record.options.dateRange && (
                <div style={{ marginBottom: '8px' }}>
                  <strong>{t('export.date_range')}:</strong> 
                  {new Date(record.options.dateRange.start).toLocaleDateString()} - 
                  {new Date(record.options.dateRange.end).toLocaleDateString()}
                </div>
              )}
              {record.options.includeMetadata && (
                <div style={{ marginBottom: '8px' }}>
                  <strong>{t('export.include_metadata')}:</strong> {t('common.yes')}
                </div>
              )}
