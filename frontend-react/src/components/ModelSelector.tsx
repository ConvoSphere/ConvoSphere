        ...
        <Row gutter={16} align="middle">
          <Col span={showPerformance || showCosts ? 8 : 12}>
            <div>
              <div style={{ fontWeight: 500, marginBottom: 4 }}>
                ...
                {model.isFavorite && (
                  <StarOutlined
                    style={{ color: colors.colorWarning, marginLeft: 4 }}
                  />
                )}
              </div>
              <div
                style={{ fontSize: "12px", color: colors.colorTextSecondary }}
              >
            ...
            <Col span={8}>
              <div style={{ marginBottom: 4 }}>
                <Text style={{ fontSize: "12px" }}>
                  {t("model_selector.response_time")}:{" "}
                </Text>
                <Tag
          ...
          {showCosts && (
            <Col span={8}>
              <div style={{ marginBottom: 4 }}>
                <Text style={{ fontSize: "12px" }}>
                  {t("model_selector.cost")}:{" "}
                </Text>
                <Tag color={getCostColor(model.costPer1kTokens)} size="small">
                  ${model.costPer1kTokens.toFixed(4)}/1k
                </Tag>
              </div>
              <div>
                <Text style={{ fontSize: "12px" }}>
                  {t("model_selector.max_tokens")}:{" "}
                </Text>
                <Text style={{ fontSize: "12px" }}>
      ...
      render: (record: AIModel) => (
        <div>
          <div style={{ marginBottom: 8 }}>
            <Text style={{ fontSize: "12px" }}>
              {t("model_selector.comparison.response_time")}
            </Text>
            <div>
              <Tag
                color={getResponseTimeColor(record.performance.responseTime)}
              >
          ...
          <div>
            <Text style={{ fontSize: "12px" }}>
              {t("model_selector.comparison.success_rate")}
            </Text>
            <div>
              <Progress
      ...
      render: (record: AIModel) => (
        <div>
          ...
          <div style={{ marginTop: 8 }}>
            <Text style={{ fontSize: "12px" }}>
              {t("model_selector.comparison.max_tokens")}
            </Text>
    ...
    },
    {
      ...
      render: (record: AIModel) => (
        <div>
          ...
          <div style={{ marginTop: 8 }}>
            <Text style={{ fontSize: "12px" }}>
              {t("model_selector.comparison.last_used")}
            </Text>
  ...
  return (
    <div>
      ...
      >
        ...
        {activeModels
          .filter((m) => !m.isDefault && !m.isFavorite)
          .map((model) => (
            <Option key={model.id} value={model.id}>
              {renderModelOption(model)}
            </Option>
                ...
                {(() => {
                  ...
                  return (
                    <Row gutter={16}>
                      <Col span={8}>
                        <Text style={{ fontSize: "12px" }}>
                          {t("model_selector.info.provider")}:{" "}
                        </Text>
                        <Text strong>{selectedModel.provider}</Text>
                      </Col>
                      <Col span={8}>
                        <Text style={{ fontSize: "12px" }}>
                          {t("model_selector.info.max_tokens")}:{" "}
                        </Text>
                        <Text strong>
                      ...
                      <Col span={8}>
                        <Text style={{ fontSize: "12px" }}>
                          {t("model_selector.info.cost")}:{" "}
                        </Text>
                        <Text strong>
                          ${selectedModel.costPer1kTokens.toFixed(4)}/1k
                        </Text>
                      </Col>
                    </Row>
                  );
