        ...
        <div className="overall-progress">
          <Space direction="vertical" size="small" style={{ width: '100%' }}>
            ...
            <div className="progress-stats">
              <Space>
                <Text type="secondary">
                  {steps.filter(s => s.status === 'completed').length} / {steps.length} {t('chat.progress.completed')}
                </Text>
                {currentStep && (
                  <Text type="secondary">
                    {t('chat.progress.current')}: {steps.find(s => s.id === currentStep)?.name}
                  </Text>
                )}
              </Space>
            </div>
      ...
      {mode === 'agent' && (
        <div className="tool-usage-indicator">
          <Space>
            ...
            <Tag color="green">
              {steps.filter(s => s.status === 'completed').length} {t('chat.progress.toolsUsed')}
            </Tag>
          </Space>
        </div>
      )}
