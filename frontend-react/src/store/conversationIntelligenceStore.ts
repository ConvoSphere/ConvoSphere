...
import { conversationIntelligenceService, type AnalyticsFilters, type ConversationAnalytics, type SentimentAnalysis, type TopicCluster, type UserBehaviorMetrics } from '../services/conversationIntelligence';
...
interface ConversationIntelligenceState {
  ...
  exportAnalytics: (filters?: AnalyticsFilters, format?: 'csv' | 'json') => Promise<void>;
  setFilters: (filters: AnalyticsFilters) => void;
  clearError: () => void;
  reset: () => void;
}

export const useConversationIntelligenceStore = create<ConversationIntelligenceState>((set, get) => ({
  // Initial state
  analytics: null,
  ...
  fetchAnalytics: async (filters?: AnalyticsFilters) => {
    ...
    try {
      const analytics = await conversationIntelligenceService.getAnalytics(filters);
  ...
  fetchSentimentAnalysis: async (conversationId: string) => {
    set({ loading: true, error: null });
    try {
      const sentimentAnalysis = await conversationIntelligenceService.getSentimentAnalysis(conversationId);
  ...
  fetchTopicClustering: async (filters?: AnalyticsFilters) => {
    set({ loading: true, error: null });
    try {
      const topicClusters = await conversationIntelligenceService.getTopicClustering(filters);
  ...
  fetchUserBehaviorAnalysis: async (filters?: AnalyticsFilters) => {
    set({ loading: true, error: null });
    try {
      const userBehavior = await conversationIntelligenceService.getUserBehaviorAnalysis(filters);
  ...
  fetchRealTimeMetrics: async () => {
    try {
      const realTimeMetrics = await conversationIntelligenceService.getRealTimeMetrics();
      set({ realTimeMetrics });
    } catch (error: any) {
  ...
  exportAnalytics: async (filters?: AnalyticsFilters, format: 'csv' | 'json' = 'csv') => {
    try {
      const blob = await conversationIntelligenceService.exportAnalytics(filters, format);
