...
export const useAIModelsStore = create<AIModelsState>((set, get) => ({
  ...
  toggleModelActive: async (modelId: string) => {
    try {
      ...
      const response = await fetch(`${config.apiUrl}${config.apiEndpoints.assistants}/models/${modelId}/toggle`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  testModel: async (modelId: string, prompt: string) => {
    try {
      set({ loading: true, error: null });
      const response = await fetch(`${config.apiUrl}${config.apiEndpoints.assistants}/models/${modelId}/test`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
