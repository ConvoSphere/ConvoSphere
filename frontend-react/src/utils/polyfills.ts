...
export const featureDetection = {
  ...
  device: {
    ...
    lowBandwidth: (navigator as any).connection?.effectiveType === 'slow-2g' || (navigator as any).connection?.effectiveType === '2g',
...
export const progressiveEnhancement = {
  ...
  applyFeatureClasses() {
    ...
    if (featureDetection.css.customProperties) html.classList.add('css-custom-properties');
    if (featureDetection.css.backdropFilter) html.classList.add('css-backdrop-filter');
    if (featureDetection.css.aspectRatio) html.classList.add('css-aspect-ratio');
    ...
    if (featureDetection.js.serviceWorkers) html.classList.add('js-service-workers');
    
    // Web APIs
    if (featureDetection.web.fetch) html.classList.add('web-fetch');
    if (featureDetection.web.intersectionObserver) html.classList.add('web-intersection-observer');
    if (featureDetection.web.resizeObserver) html.classList.add('web-resize-observer');
    if (featureDetection.web.performanceObserver) html.classList.add('web-performance-observer');
    if (featureDetection.web.requestIdleCallback) html.classList.add('web-request-idle-callback');
    ...
    if (featureDetection.device.lowBandwidth) html.classList.add('device-low-bandwidth');
    if (featureDetection.device.saveData) html.classList.add('device-save-data');
  },
  ...
  shouldLoadHeavyFeatures() {
    return !featureDetection.device.lowBandwidth && 
