...
const Tools: React.FC = () => {
  ...
  const categories = [
    { value: 'utility', label: t('tools.categories.utility', 'Utility'), icon: <ToolOutlined /> },
    { value: 'api', label: t('tools.categories.api', 'API'), icon: <ApiOutlined /> },
    { value: 'calculation', label: t('tools.categories.calculation', 'Calculation'), icon: <CalculatorOutlined /> },
    { value: 'file', label: t('tools.categories.file', 'File'), icon: <FileTextOutlined /> },
    { value: 'code', label: t('tools.categories.code', 'Code'), icon: <CodeOutlined /> },
  ];
  ...
  const handleRunTool = async () => {
    ...
    try {
      ...
      setVisible(false);
      form.resetFields();
      message.success(t('tools.execution_success', 'Tool erfolgreich ausgeführt'));
    } catch (error) {
      message.error(t('tools.execution_failed', 'Fehler bei der Tool-Ausführung'));
    } finally {
      setRunning(false);
    }
  };
  ...
  const handleToggleActive = async (tool: Tool) => {
    try {
      ...
      message.success(tool.isActive ? t('tools.deactivated', 'Tool deaktiviert') : t('tools.activated', 'Tool aktiviert'));
    } catch (error) {
      message.error(t('tools.toggle_failed', 'Fehler beim Umschalten des Tools'));
    }
  };
                  ...
                  header={
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <ModernButton
                        ...
                        size="md"
                        icon={<PlusOutlined />}
                        onClick={() => message.info(t('tools.add_tool', 'Tool hinzufügen'))}
                      >
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <Paragraph 
                                ellipsis={{ rows: 2 }} 
                                style={{ margin: 0, color: colors.colorTextSecondary, fontSize: '14px' }}
                              >
                                {tool.description}
                              </Paragraph>
        ...
        <Modal
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              {selectedTool ? t('tools.run_tool', 'Tool ausführen: {{name}}', { name: selectedTool.name }) : t('tools.run_tool', 'Tool ausführen')}
            </div>
          }
          open={visible}
          onCancel={() => {
          ...
          >
            {selectedTool?.parameters.map((param) => (
              <ModernFormItem
                ...
                label={param.name}
                rules={[
                  ...(param.required ? [{ required: true, message: t('tools.parameter_required', 'Parameter ist erforderlich') }] : []),
                ]}
