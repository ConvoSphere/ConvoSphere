...
import { Row, Col, Space, Typography, Modal, message, Spin, Avatar, Tag, Empty, Statistic } from 'antd';
...
const McpTools: React.FC = () => {
  ...
  const getToolIcon = (toolName: string) => {
    ...
    if (name.includes('database') || name.includes('db')) return <DatabaseOutlined />;
    ...
    <ModernCard key={tool.id} variant="interactive" size="md">
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <ModernButton
          ...
          onClick={() => { setSelected(tool); setVisible(true); }}
        >
          {t('common.run')}
        </ModernButton>
      </div>
          ...
          <ModernCard key={exec.id} variant="outlined" size="sm">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <Text strong style={{ fontSize: '14px' }}>{exec.toolName}</Text>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {exec.duration && (
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <ModernCard variant="gradient" size="lg" className="stagger-children">
          <div style={{ textAlign: 'center', padding: '32px 0' }}>
            ...
            <Title level={1} style={{ color: '#FFFFFF', margin: 0 }}>{t('mcp_tools.title')}</Title>
        ...
        <Row gutter={[24, 24]} style={{ marginTop: 32 }}>
            ...
            <ModernCard variant="elevated" size="lg">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <Title level={3} style={{ margin: 0 }}>
                  <ToolOutlined style={{ marginRight: 8, color: colors.colorPrimary }} />
                  {t('mcp_tools.available_tools')}
                </Title>
                <Tag color="blue">{filteredTools.length} {t('mcp_tools.tools')}</Tag>
              </div>

              {loading ? (
                <div style={{ textAlign: 'center', padding: '40px' }}>
                  <Spin size="large" />
                  <Text style={{ display: 'block', marginTop: 16 }}>{t('mcp_tools.loading')}</Text>
                </div>
              ) : filteredTools.length === 0 ? (
                <Empty 
                  description={searchQuery ? t('mcp_tools.no_results') : t('mcp_tools.no_tools')} 
          ...
          <Col xs={24} lg={8}>
            <ModernCard variant="interactive" size="md" style={{ marginBottom: 24 }}>
              <Title level={4}>
                <ThunderboltOutlined style={{ marginRight: 8, color: colors.colorPrimary }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Text strong style={{ color: colors.colorSuccess }}>
                    {executionHistory.filter(exec => exec.status === 'success').length}
                  </Text>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Text>{t('mcp_tools.stats.failed_executions')}</Text>
                  <Text strong style={{ color: colors.colorError }}>
                    {executionHistory.filter(exec => exec.status === 'error').length}
                  </Text>
                </div>
              </div>
            </ModernCard>
            ...
            <ModernCard variant="outlined" size="md" style={{ marginBottom: 24 }}>
              <Title level={4}>
                <SettingOutlined style={{ marginRight: 8, color: colors.colorPrimary }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <ModernButton variant="secondary" icon={<HistoryOutlined />} block>
            ...
            <ModernCard variant="elevated" size="md">
              <Title level={4}>
                <HistoryOutlined style={{ marginRight: 8, color: colors.colorPrimary }} />
      ...
      >
        ...
        <ModernForm layout="vertical" onFinish={handleRun}>
          <ModernFormItem 
            label={t('mcp_tools.parameter_label')}
            required
          >
            <ModernInput
              value={param}
