...
const Assistants: React.FC = () => {
  ...
  const availableModels = [
    ...
    { value: 'claude-3-sonnet', label: 'Claude 3 Sonnet', description: 'Fast and efficient' }
  ];
  ...
  const availableTools = [
    { id: 'tool1', name: 'Search Engine', description: 'Web search capabilities' },
    { id: 'tool2', name: 'Calculator', description: 'Mathematical calculations' },
    { id: 'tool3', name: 'Code Interpreter', description: 'Code execution and analysis' },
    { id: 'tool4', name: 'File Manager', description: 'File system operations' },
    { id: 'tool5', name: 'Image Generator', description: 'Create images from text' }
  ];
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setAssistants(prev => prev.map(a => a.id === editingAssistant.id ? updatedAssistant : a));
      setVisible(false);
  ...
  const handleToggleActive = async (assistant: Assistant) => {
    try {
      ...
      setAssistants(prev => prev.map(a => a.id === assistant.id ? updatedAssistant : a));
      message.success(assistant.isActive ? t('assistants.deactivated') : t('assistants.activated'));
    } catch (error) {
  ...
  return (
    <div style={{ padding: '24px 0' }}>
            ...
            renderItem={(assistant) => (
              <List.Item>
                <Card
                  ...
                  actions={[
                    ...
                    <Tooltip title={assistant.isActive ? t('assistants.actions.deactivate') : t('assistants.actions.activate')}>
                      <Button 
                        type="text" 
                        icon={assistant.isActive ? <PauseCircleOutlined /> : <PlayCircleOutlined />}
                ...
                >
                  <List.Item.Meta
                    avatar={
                      <Badge status={assistant.isActive ? 'success' : 'default'}>
                        <Avatar 
                          size={48} 
                          icon={<RobotOutlined />}
                          style={{ backgroundColor: colors.colorPrimary }}
      ...
      <Modal
        open={visible}
        title={editingAssistant ? t('assistants.edit_title') : t('assistants.add_title')}
        onCancel={() => {
          setVisible(false);
          setEditingAssistant(null);
          form.resetFields();
      ...
      >
        <Form form={form} layout="vertical">
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item 
                ...
                rules={[{ required: true, message: t('assistants.form.name_required') }]}
              >
                <Input placeholder={t('assistants.form.name_placeholder')} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="model" label={t('assistants.form.model')} rules={[{ required: true }]}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="temperature" label={t('assistants.form.temperature')}>
            ...
            <Col span={12}>
              <Form.Item name="isActive" label={t('assistants.form.active')} valuePropName="checked">
                <Switch />
              </Form.Item>
            </Col>
          </Row>

          <Form.Item name="knowledgeBaseIds" label={t('assistants.form.knowledge_bases')}>
            <Select mode="multiple" placeholder={t('assistants.form.knowledge_bases_placeholder')}>
          ...
          <Form.Item name="toolIds" label={t('assistants.form.tools')}>
            <Select mode="multiple" placeholder={t('assistants.form.tools_placeholder')}>
