...
export const useChat = () => {
  ...
  const sendMessage = useCallback(async (formData: ChatFormData) => {
  ...
  const sendMessageStreaming = useCallback(async (formData: ChatFormData) => {
    if (!formData.message.trim()) return;
  ...
  const exportChat = useCallback(async (format: 'json' | 'txt' | 'md' | 'pdf') => {
    try {
      const exportData = {
        thread,
        messages,
