...
const Conversations: React.FC = () => {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      message.error(t('conversations.load_failed', 'Fehler beim Laden der Konversationen'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelect = async (id: number) => {
    ...
    try {
      ...
      message.error(t('conversations.load_detail_failed', 'Fehler beim Laden der Details'));
      setConvDetail(null);
    } finally {
  ...
  const formatDate = (dateString: string) => {
    ...
    if (diffDays === 1) {
      ...
      return t('conversations.days_ago', '{{days}} Tage her', { days: diffDays - 1 });
    } else {
        ...
        <ModernCard variant="gradient" size="lg" className="stagger-children">
          <div style={{ textAlign: 'center', padding: '32px 0' }}>
            ...
            <Text style={{ fontSize: '18px', color: 'rgba(255, 255, 255, 0.9)' }}>
              {t('conversations.subtitle', 'Verwalten Sie Ihre Gespräche und Nachrichten')}
            </Text>
          </div>
        </ModernCard>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <ModernCard variant="elevated" size="md">
                  <Row gutter={[16, 16]} align="middle">
                    <Col xs={24} md={12}>
                      <ModernInput
                        placeholder={t('conversations.search_placeholder', 'Konversationen durchsuchen...')}
                    ...
                    <Col xs={12} md={6}>
                      <ModernSelect
                        value={filterType}
                        onChange={(value) => handleFilterChange('conversation', value)}
                        style={{ width: '100%' }}
                      >
                        <ModernSelect.Option value="all">{t('conversations.filter_all', 'Alle Typen')}</ModernSelect.Option>
                        <ModernSelect.Option value="chat">{t('conversations.filter_chat', 'Chat')}</ModernSelect.Option>
                        <ModernSelect.Option value="assistant">{t('conversations.filter_assistant', 'Assistent')}</ModernSelect.Option>
                        <ModernSelect.Option value="knowledge">{t('conversations.filter_knowledge', 'Wissen')}</ModernSelect.Option>
                        <ModernSelect.Option value="tool">{t('conversations.filter_tool', 'Tools')}</ModernSelect.Option>
                      </ModernSelect>
                    </Col>
                    ...
                    <Col xs={12} md={6}>
                      <ModernSelect
                        ...
                        style={{ width: '100%' }}
                      >
                        <ModernSelect.Option value="all">{t('conversations.status_all', 'Alle Status')}</ModernSelect.Option>
                        <ModernSelect.Option value="active">{t('conversations.status_active', 'Aktiv')}</ModernSelect.Option>
                        <ModernSelect.Option value="archived">{t('conversations.status_archived', 'Archiviert')}</ModernSelect.Option>
                      </ModernSelect>
                    </Col>
                  </Row>
                </ModernCard>
                  ...
                  header={
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <ModernButton
                        ...
                        size="md"
                        icon={<PlusOutlined />}
                        onClick={() => message.info(t('conversations.new_conversation', 'Neue Konversation'))}
                      >
                ...
                >
                  {loading ? (
                    ...
                    <Empty
                      description={t('conversations.no_conversations', 'Keine Konversationen gefunden')}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      {filteredConversations.map((conv) => (
                        <ModernCard
                          key={conv.id}
                          variant={selected === conv.id ? "interactive" : "outlined"}
                          size="md"
                          hoverable
                          onClick={() => handleSelect(conv.id)}
                          style={{ 
                            cursor: 'pointer',
                            border: selected === conv.id ? `2px solid ${colors.colorPrimary}` : undefined
                          }}
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                            <Avatar 
                              ...
                              style={{ 
                                backgroundColor: getConversationTypeColor(conv.type),
                                color: '#FFFFFF'
                              }}
                            />
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <Tag color={getStatusColor(conv.status)} size="small">
                                    {t(`conversations.status_${conv.status}`, conv.status)}
                                  </Tag>
                              ...
                              <Paragraph 
                                ellipsis={{ rows: 2 }} 
                                style={{ margin: 0, color: colors.colorTextSecondary }}
                              >
                                {conv.lastMessage}
                              </Paragraph>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8 }}>
                                ...
                                <Text type="secondary" style={{ fontSize: '12px' }}>
                                  ...
                                  {conv.participants} {t('conversations.participants', 'Teilnehmer')}
                                </Text>
                              </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              ...
                              <ModernButton
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  message.info(t('conversations.delete_confirm', 'Löschen bestätigen'));
                                }}
                              />
                            </div>
                          </div>
                ...
                {selected && (
                  ...
                  >
                    {loadingConv ? (
                      ...
                      <Empty description={t('conversations.no_details', 'Keine Details verfügbar')} />
                    )}
                  </ModernCard>
                )}
              </div>
                ...
                <ModernCard variant="interactive" size="md">
                  ...
                  <Space direction="vertical" size="large" style={{ width: '100%' }}>
                    ...
                    <Statistic
                      ...
                      value={stats.active}
                      prefix={<UserOutlined style={{ color: colors.colorSecondary }} />}
                      valueStyle={{ color: colors.colorSecondary, fontSize: '1.2rem' }}
                    />
                    ...
                    <Statistic
                      ...
                      valueStyle={{ color: colors.colorAccent, fontSize: '1.2rem' }}
                    />
                  </Space>
                </ModernCard>
                  ...
                  <Space direction="vertical" size="small" style={{ width: '100%' }}>
                    ...
                    {filterStatus !== 'all' && (
                      <div style={{ 
                        ...
                        <Text>{t('conversations.status_filter', 'Status')}</Text>
                        <Text strong style={{ color: colors.colorAccent }}>
                          {t(`conversations.status_${filterStatus}`, filterStatus)}
                        </Text>
                      </div>
                    )}
