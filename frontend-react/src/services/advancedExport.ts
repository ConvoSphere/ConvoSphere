...
class AdvancedExportService {
  async exportToPDF(
    ...
    try {
      ...
      if (options.includeHeader !== false) {
        ...
        pdf.text(conversationTitle || "Chat Export", pageWidth / 2, yPosition, {
          align: "center",
        });
        yPosition += 15;
      ...
      if (options.includeFooter !== false) {
        ...
        pdf.setTextColor(150, 150, 150);
        pdf.text(
          "Generated by AI Assistant Platform",
          pageWidth / 2,
          pageHeight - 10,
          { align: "center" },
        );
  ...
  async batchExport(options: BatchExportOptions): Promise<void> {
    try {
      ...
      if (zipFiles) {
        await this.createZippedBatchExport(
          conversations,
          format,
          includeMetadata,
          includeTimestamps,
        ...
        await this.createIndividualBatchExport(
          conversations,
          format,
          includeMetadata,
          includeTimestamps,
  ...
  async scheduleExport(
    ...
    try {
      ...
      const schedules = JSON.parse(
        localStorage.getItem("export-schedules") || "[]",
      );
      schedules.push({
        id: scheduleId,
        conversationId,
  ...
  private async createIndividualBatchExport(
    ...
    for (const conversation of conversations) {
      try {
        ...
        console.error(
          `Error exporting conversation ${conversation.id}:`,
          error,
        );
      }
    }
