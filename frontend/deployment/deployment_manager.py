...
class DeploymentManager:
    ...
    def deploy_to_environment(self, build_path: Path, config: DeploymentConfig, deployment_info: Dict[str, Any]):
        """Deploy to target environment."""
        ...
        deployment_info["steps"].append(f"Deployed to {config.target_environment.value}")
    
    def configure_environment(self, target_path: Path, environment: DeploymentEnvironment):
    ...
    def cleanup_old_deployments(self, keep_count: int = 5):
        ...
        sorted_deployments = sorted(self.deployment_history, key=lambda x: x["timestamp"])
