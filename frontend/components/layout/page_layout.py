...
class PageLayout:
    ...
    def __init__(self):
        ...
        # Callbacks
        self.on_sidebar_toggle: Callable | None = None
        self.on_navigation: Callable | None = None

    def create_layout(
        self, page_title: str = "", show_sidebar: bool = True,
    ) -> ui.element:
        """
    ...
    def _create_main_content(self) -> ui.element:
        ...
        with ui.element("main").classes(
            f"flex-1 {sidebar_class} transition-all duration-300",
        ) as main:
            ...
            with ui.element("div").classes("max-w-7xl mx-auto py-4 px-6"):
                with ui.element("div").classes("flex justify-between items-center"):
                    ...
                    with ui.element("div").classes("flex space-x-6"):
                        ui.link("Datenschutz", "#privacy").classes(
                            "text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200",
                        )
                        ui.link("Impressum", "#imprint").classes(
                            "text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200",
                        )
                        ui.link("Hilfe", "#help").classes(
                            "text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200",
                        )
...
class AuthLayout:
    ...
    def _create_main_content(self) -> ui.element:
        ...
        with ui.element("main").classes(
            "flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8",
        ) as main:
