...
def create_header(
    ...
    """
    with ui.element("header").classes("bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700") as header:
        with ui.element("div").classes("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"):
            with ui.element("div").classes("flex justify-between items-center py-4"):
                ...
                with ui.element("div").classes("flex items-center space-x-4"):
                    ...
                    # Page title
                    if title:
                        with ui.element("div").classes("ml-4 pl-4 border-l border-gray-300 dark:border-gray-600"):
                            ui.html(f"<span style='color: var(--color-text-secondary); font-size: 18px;'>{title}</span>")
...
def _handle_theme_toggle(callback: Optional[Callable] = None):
    """Handle theme toggle."""
...
def _create_user_menu():
    ...
    with ui.element("div").classes("relative"):
        ...
        with ui.element("div").classes("absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 z-50 hidden") as user_menu:
            # Profile link
            ui.link(
                "Profil",
                "#profile",
                on_click=lambda: _handle_menu_item("profile")
            ).classes("block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700")
