...
from services.user_service import UserProfile, UserStats, UserPreferences, UserRole, UserStatus
from utils.helpers import format_timestamp, format_file_size, format_relative_time
from utils.constants import SUPPORTED_AVATAR_TYPES
...
class ProfileCard:
    ...
    def __init__(
        self,
        user: UserProfile,
        on_edit: Optional[Callable[[UserProfile], None]] = None,
        on_avatar_upload: Optional[Callable[[bytes, str], None]] = None,
        on_password_change: Optional[Callable[[str, str], None]] = None,
        on_preferences_update: Optional[Callable[[UserPreferences], None]] = None,
        ...
        with ui.row().classes("items-start space-x-6 mb-6"):
            ...
            with ui.column().classes("flex-1"):
                with ui.row().classes("items-center space-x-4 mb-2"):
                    ui.label(f"{self.user.first_name} {self.user.last_name}").classes("text-2xl font-bold")
                    
                    # Role badge
                    role_config = {
                        UserRole.ADMIN: {"color": "bg-red-100 text-red-800", "text": "Admin"},
                        UserRole.MODERATOR: {"color": "bg-orange-100 text-orange-800", "text": "Moderator"},
                        UserRole.USER: {"color": "bg-blue-100 text-blue-800", "text": "User"}
                    }
                    
                    config = role_config.get(self.user.role, role_config[UserRole.USER])
                    ui.label(config["text"]).classes(f"px-3 py-1 rounded-full text-xs font-medium {config['color']}")
                ...
                # Status
                status_config = {
                    UserStatus.ACTIVE: {"color": "text-green-600", "icon": "check_circle", "text": "Aktiv"},
                    UserStatus.INACTIVE: {"color": "text-gray-600", "icon": "pause_circle", "text": "Inaktiv"},
                    UserStatus.SUSPENDED: {"color": "text-red-600", "icon": "block", "text": "Gesperrt"},
                    UserStatus.PENDING: {"color": "text-yellow-600", "icon": "schedule", "text": "Ausstehend"}
                }
                
                config = status_config.get(self.user.status, status_config[UserStatus.ACTIVE])
    ...
    def create_avatar_section(self):
        ...
        with self.avatar_container:
            ...
            if self.user.avatar_url:
                ui.image(self.user.avatar_url).classes("w-24 h-24 rounded-full object-cover")
            else:
                # Default avatar
                with ui.element("div").classes("w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center"):
            ...
            ).classes("absolute bottom-0 right-0 w-8 h-8 bg-blue-500 text-white rounded-full")
        ...
        with ui.element("div").classes("mb-6"):
            ...
            with ui.row().classes("space-x-6"):
                ...
                if self.user.location:
                    with ui.column():
                        ui.label("Standort").classes("text-sm font-medium text-gray-700")
                ...
                if self.user.website:
                    with ui.column():
                        ...
                        ui.link(self.user.website, self.user.website).classes("text-sm text-blue-600")
            ...
            with ui.expansion("Account-Informationen", icon="info").classes("mt-4"):
                with ui.column().classes("space-y-2"):
                    with ui.row().classes("justify-between"):
                        ...
                        ui.label(format_timestamp(self.user.created_at)).classes("text-sm")
                    
                    with ui.row().classes("justify-between"):
                        ui.label("Letzte Aktivität:").classes("text-sm text-gray-600")
                        if self.user.last_login:
                            ui.label(format_relative_time(self.user.last_login)).classes("text-sm")
                        else:
                            ui.label("Nie").classes("text-sm text-gray-400")
                    
                    with ui.row().classes("justify-between"):
                        ui.label("Zuletzt aktualisiert:").classes("text-sm text-gray-600")
                        ui.label(format_relative_time(self.user.updated_at)).classes("text-sm")
    ...
    def show_avatar_upload(self):
        ...
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                preview_container = ui.element("div").classes("w-32 h-32 border rounded-lg overflow-hidden")
                
                # Buttons
                with ui.row().classes("justify-end space-x-2"):
                    ui.button("Abbrechen", on_click=dialog.close).classes("bg-gray-500 text-white")
                    ui.button("Hochladen", on_click=dialog.close).classes("bg-blue-600 text-white")
                ...
                with ui.expansion("Erscheinungsbild", icon="palette"):
                    with ui.column().classes("space-y-4"):
                        theme_select = ui.select(
                            ...
                            value=self.user.preferences.theme if self.user.preferences else "light"
                        ).classes("w-full")
                        
                        font_size_select = ui.select(
                            "Schriftgröße",
                            options=["small", "medium", "large"],
                            value=self.user.preferences.font_size if self.user.preferences else "medium"
                        ).classes("w-full")
                        
                        ui.switch(
                            "Kompakter Modus",
                            value=self.user.preferences.ui_compact_mode if self.user.preferences else False
                        ).classes("w-full")
                        
                        ui.switch(
                            "Hoher Kontrast",
                            value=self.user.preferences.high_contrast if self.user.preferences else False
                        ).classes("w-full")
                ...
                with ui.expansion("Benachrichtigungen", icon="notifications"):
                    with ui.column().classes("space-y-4"):
                        ui.switch(
                            ...
                            value=self.user.preferences.notifications_enabled if self.user.preferences else True
                        ).classes("w-full")
                        
                        ui.switch(
                            "E-Mail-Benachrichtigungen",
                            value=self.user.preferences.email_notifications if self.user.preferences else True
                        ).classes("w-full")
                        
                        ui.switch(
                            "Push-Benachrichtigungen",
                            value=self.user.preferences.push_notifications if self.user.preferences else False
                        ).classes("w-full")
                ...
                with ui.expansion("Chat-Einstellungen", icon="chat"):
                    with ui.column().classes("space-y-4"):
                        history_limit = ui.number(
                            "Chat-Verlauf-Limit",
                            value=self.user.preferences.chat_history_limit if self.user.preferences else 100,
                            min=10,
                            max=1000
                        ).classes("w-full")
                        ...
                        ui.switch(
                            "Auto-Save",
                            value=self.user.preferences.auto_save if self.user.preferences else True
                        ).classes("w-full")
                        
                        ui.switch(
                            "Sound aktivieren",
                            value=self.user.preferences.sound_enabled if self.user.preferences else True
                        ).classes("w-full")
    ...
    def show_admin_actions_dialog(self):
        ...
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.row().classes("justify-end space-x-2"):
                    ui.button("Abbrechen", on_click=dialog.close).classes("bg-gray-500 text-white")
                    ui.button("Speichern", on_click=dialog.close).classes("bg-blue-600 text-white")
    ...
    def update_stats(self, stats: UserStats):
        ...
        with self.stats_container:
            with ui.row().classes("grid grid-cols-2 md:grid-cols-4 gap-4"):
                ...
                with ui.element("div").classes("bg-blue-50 border rounded-lg p-4"):
                    ui.label("Gespräche").classes("text-sm text-gray-600")
                    ui.label(str(stats.total_conversations)).classes("text-2xl font-bold text-blue-600")
                
                # Messages
                with ui.element("div").classes("bg-green-50 border rounded-lg p-4"):
                    ui.label("Nachrichten").classes("text-sm text-gray-600")
                    ui.label(str(stats.total_messages)).classes("text-2xl font-bold text-green-600")
                
                # Documents
                with ui.element("div").classes("bg-purple-50 border rounded-lg p-4"):
                    ui.label("Dokumente").classes("text-sm text-gray-600")
                    ui.label(str(stats.total_documents)).classes("text-2xl font-bold text-purple-600")
                
                # Storage
                with ui.element("div").classes("bg-orange-50 border rounded-lg p-4"):
                    ui.label("Speicher").classes("text-sm text-gray-600")
                    ui.label(format_file_size(stats.storage_used)).classes("text-2xl font-bold text-orange-600")
