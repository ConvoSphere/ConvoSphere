...
class AuthForm:
    ...
    def _create_header(self):
        ...
        with ui.element("div").classes("text-center mb-8"):
            if self.form_type == "login":
                ui.html("<h1 style='font-size: 28px; font-weight: 700; color: var(--color-text); margin-bottom: 8px;'>Willkommen zu ConvoSphere</h1>")
                ui.html("<p style='color: var(--color-text-secondary); font-size: 16px;'>Melde dich in deinem Konto an</p>")
            else:
                ui.html("<h1 style='font-size: 28px; font-weight: 700; color: var(--color-text); margin-bottom: 8px;'>Konto erstellen</h1>")
                ui.html("<p style='color: var(--color-text-secondary); font-size: 16px;'>Erstelle dein ConvoSphere-Konto</p>")
    ...
    def _create_footer(self):
        ...
        with ui.element("div").classes("text-center"):
            if self.form_type == "login":
                ui.html("<span style='color: var(--color-text-secondary);'>Noch kein Konto? </span>")
                ui.link("Jetzt registrieren", "#register").classes("text-blue-600 hover:text-blue-500 font-medium")
            else:
                ui.html("<span style='color: var(--color-text-secondary);'>Bereits ein Konto? </span>")
                ui.link("Jetzt anmelden", "#login").classes("text-blue-600 hover:text-blue-500 font-medium")
    ...
    def _set_loading(self, loading: bool):
        ...
        if loading:
            ...
            self.submit_button.classes("w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors")
    ...
    async def _handle_register(self) -> bool:
        ...
        if success:
            ui.notify("Registrierung erfolgreich! Du kannst dich jetzt anmelden.", type="positive")
