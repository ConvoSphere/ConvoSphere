...
from utils.helpers import format_file_size, validate_file_type
...
class AdvancedUploadComponent:
    ...
    def __init__(
        ...
        on_complete: Optional[Callable[[List[Document]], None]] = None
    ):
        """
    ...
    def create_drop_zone(self):
        ...
        with self.drop_zone:
            ...
            ui.label("Dateien hier hineinziehen oder klicken zum Auswählen").classes("text-lg text-gray-600 mb-2")
            ui.label("Unterstützte Formate: PDF, DOCX, TXT, MD, HTML, JSON, CSV, Bilder, Audio, Video").classes("text-sm text-gray-500")
            ...
            ).props("accept=.pdf,.docx,.txt,.md,.html,.json,.csv,.jpg,.jpeg,.png,.gif,.mp3,.mp4,.avi,.mov")
    ...
    def update_file_list(self):
        ...
        if not self.upload_queue:
            with self.file_list:
                ui.label("Keine Dateien ausgewählt").classes("text-gray-500 text-center py-4")
            return
        ...
        with ui.element("div").classes("bg-white border rounded-lg p-4"):
            with ui.row().classes("items-center justify-between"):
                ...
                with ui.column():
                    ...
                    ui.label(f"{format_file_size(file_info['size'])} • {file_info['type']}").classes("text-sm text-gray-600")
    ...
    async def start_batch_upload(self):
        ...
        with self.progress_container:
            ...
            # Progress label
            self.progress_label = ui.label("Bereite Upload vor...").classes("text-sm text-gray-600")
        ...
        try:
            ...
            for i, file_info in enumerate(self.upload_queue):
                ...
                # Update progress
                self.progress_label.text = f"Lade hoch: {file_info['name']} ({i+1}/{total_files})"
                ...
                try:
                    ...
                    document_data = {
                        ...
                        "description": f"Uploaded via batch upload",
                        "category": "Upload",
                        "tags": ["batch-upload"]
                    }
            ...
            # Upload complete
            self.progress_label.text = f"Upload abgeschlossen: {completed_files}/{total_files} erfolgreich"
            ...
            with ui.column().classes("space-y-4"):
                ...
                if self.completed_uploads:
                    with ui.row().classes("items-center space-x-2"):
                        ...
                        ui.label(f"{len(self.completed_uploads)} Dateien erfolgreich hochgeladen").classes("text-green-600")
                
                # Error count
                if self.failed_uploads:
                    with ui.row().classes("items-center space-x-2"):
                        ui.icon("error").classes("w-5 h-5 text-red-500")
                        ui.label(f"{len(self.failed_uploads)} Dateien fehlgeschlagen").classes("text-red-600")
        ...
        with ui.dialog() as dialog, ui.card().classes("w-full max-w-lg"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.expansion("Chunking-Optionen", icon="content_cut"):
                    ui.number("Chunk-Größe", value=500, min=100, max=2000).classes("w-full")
                    ui.number("Chunk-Überlappung", value=50, min=0, max=200).classes("w-full")
                
                # Embedding options
                with ui.expansion("Embedding-Optionen", icon="memory"):
                    ui.select("Embedding-Modell", options=["text-embedding-ada-002", "text-embedding-3-small"]).classes("w-full")
