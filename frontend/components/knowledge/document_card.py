...
class DocumentCard:
    ...
    def __init__(
        self,
        document: Document,
        on_edit: Optional[Callable[[Document], None]] = None,
        on_delete: Optional[Callable[[Document], None]] = None,
        on_download: Optional[Callable[[Document], None]] = None,
        on_reprocess: Optional[Callable[[Document], None]] = None,
        on_view_chunks: Optional[Callable[[Document], None]] = None
    ):
        """
        ...
        with ui.row().classes("items-center justify-between mb-4"):
            ...
            with ui.column():
                ...
                with ui.row().classes("items-center space-x-2"):
                    ...
                    ui.label(self.document.file_type.value.upper()).classes("text-sm text-gray-500")
    ...
    def create_status_indicator(self):
        ...
        config = status_config.get(self.document.status, status_config[DocumentStatus.FAILED])
        ...
        with ui.column().classes("items-end"):
            ...
            if self.document.status in [DocumentStatus.UPLOADING, DocumentStatus.PROCESSING]:
    ...
    def create_document_info(self):
        ...
        with ui.column().classes("space-y-2 mb-4"):
            ...
            with ui.row().classes("items-center justify-between text-sm text-gray-600"):
                ...
                ui.label(f"Hochgeladen: {format_relative_time(self.document.uploaded_at)}")
            ...
            if self.document.description:
                ui.label(self.document.description).classes("text-sm text-gray-700 mt-2")
    
    def create_metadata(self):
        """Create metadata section."""
        has_metadata = (
            self.document.category or 
            self.document.tags or 
            self.document.chunks or 
    ...
    def create_actions(self):
        ...
        with ui.row().classes("space-x-2"):
            ...
            # Reprocess button
            if self.document.status in [DocumentStatus.COMPLETED, DocumentStatus.FAILED]:
                ui.button(
                    "Neu verarbeiten",
                    icon="refresh",
