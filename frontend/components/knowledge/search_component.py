...
from services.knowledge_service import SearchResult, knowledge_service
...
class AdvancedSearchComponent:
    ...
    def __init__(
        self,
        on_search: Callable[[list[SearchResult]], None] | None = None,
    ...
    def create_advanced_filters(self):
        ...
        self.filters_container = ui.element("div").classes(
            "mb-4 p-4 bg-gray-50 rounded-lg hidden",
        )

        with self.filters_container:
            ui.label(t("knowledge.advanced_filters")).classes(
    ...
    def create_search_suggestions(self):
        ...
        with self.suggestions_container:
            ui.label(t("knowledge.search_suggestions")).classes(
    ...
    def create_search_results(self):
        ...
        with self.results_container:
            # Results header
            self.results_header = ui.element("div").classes(
                "flex items-center justify-between p-4 bg-gray-50 rounded",
            )
        ...
        with self.results_container:
            ...
            with ui.element("div").classes(
                ...
                if self.search_results:
                    ui.label(
                        f"{len(self.search_results)} {t('knowledge.results_found')}",
                    )
                else:
            ...
            with ui.row().classes("items-start justify-between"):
                ...
                with ui.column().classes("flex-1"):
                    ...
                    with ui.row().classes("items-center space-x-2 mb-2"):
                        ...
                        ui.badge(f"{result.score:.2f}").classes(
                    ...
                    with ui.row().classes(
                        ...
                        ui.label(
                            f"Position {result.start_position}-{result.end_position}",
                        )
                ...
                with ui.column().classes("space-y-2"):
                    for query in reversed(self.search_history):
                        ui.button(
                            ...
                            "w-full text-left justify-start bg-gray-50 hover:bg-gray-100",
                        )
            else:
                ui.label(t("knowledge.no_search_history")).classes(
...
def create_advanced_search_component(
    ...
    return AdvancedSearchComponent(
        on_search=on_search, on_filter_change=on_filter_change,
    )
