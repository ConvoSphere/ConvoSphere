...
from services.message_service import AdvancedMessage, message_service
from services.tool_service import Tool, tool_service
from utils.constants import MAX_FILE_SIZE, SUPPORTED_FILE_TYPES
...
class ChatInput:
    ...
    def __init__(
        self,
        conversation_id: str,
        on_message_sent: Callable[[AdvancedMessage], None] | None = None,
        on_file_upload: Callable[[AdvancedMessage], None] | None = None,
        on_tool_executed: Callable[[AdvancedMessage], None] | None = None,
        ...
        self.reply_to_message: str | None = None
    ...
    def create_chat_input(self):
        ...
        with self.container:
            # Reply indicator
            self.reply_indicator = ui.element("div").classes(
                "hidden mb-2 p-2 bg-blue-50 rounded",
            )
            ...
            # Tool selection
            self.tool_selection = ui.element("div").classes(
                "hidden mb-2 p-2 bg-gray-50 rounded",
            )
    ...
    def parse_tool_input(self, content: str) -> dict[str, Any] | None:
    ...
    async def handle_file_upload(self, event):
        ...
        for file in event.files:
            if file.size > MAX_FILE_SIZE:
                ui.notify(
                    f"Datei {file.name} ist zu groß (max {MAX_FILE_SIZE} bytes)",
                    type="error",
                )
                continue
    ...
    async def upload_file_with_progress(self, file_info: dict[str, Any]):
        ...
        try:
            ...
            ui.notify(
                f"Upload fehlgeschlagen für {file_info['name']}: {str(e)}", type="error",
            )
    ...
    def update_file_preview(self):
        ...
        with self.file_preview:
            ...
            for i, file_info in enumerate(self.selected_files):
                with ui.row().classes(
                    "items-center justify-between p-2 border rounded",
                ):
                    # File info
                    with ui.column().classes("flex-1"):
                        ui.label(f"• {file_info['name']}").classes(
                            "text-sm font-medium",
                        )
                        ...
                        if file_info["upload_progress"] < 100.0:
                            with ui.row().classes("items-center space-x-2"):
                                ...
                                ui.label(
                                    f"{file_info['upload_progress']:.1f}%",
                                ).classes("text-xs")
                        else:
                            ui.label("✓ Hochgeladen", type="positive").classes(
                                "text-xs",
                            )
        ...
        with self.tool_selection:
            with ui.row().classes("items-center justify-between"):
                with ui.column():
                    ...
                    ui.label(self.selected_tool.description).classes(
                        "text-sm text-gray-600",
                    )
