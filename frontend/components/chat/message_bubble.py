...
from services.message_service import AdvancedMessage, MessageStatus, MessageType
...
class MessageBubble:
            ...
            for file_attachment in self.message.file_attachments:
                with ui.element("div").classes("border rounded p-2 mb-2"):
                    ...
                    with ui.row().classes("items-center space-x-2"):
                        ui.icon(self.get_file_icon(file_attachment.file_type)).classes(
                            "w-5 h-5",
                        )
                        ui.label(file_attachment.filename).classes(
                            "font-medium text-sm",
                        )

                    # File details
                    with ui.row().classes("items-center justify-between mt-1"):
                        ui.label(format_file_size(file_attachment.file_size)).classes(
                            "text-xs opacity-70",
                        )
                        ...
                        if file_attachment.url:
                            ui.button(
                                "Herunterladen",
                                icon="download",
                                on_click=lambda url=file_attachment.url: self.download_file(
                                    url,
                                ),
                ...
                with ui.element("div").classes("border rounded p-3 mb-2"):
                    ...
                    with ui.row().classes("items-center justify-between mb-2"):
                        with ui.row().classes("items-center space-x-2"):
                            ...
                            ui.label(tool_result.tool_name).classes(
                                "font-medium text-sm",
                            )
                        ...
                        ui.label(tool_result.status.title()).classes(
                            f"text-xs {status_color}",
                        )

                    # Execution time
                    ui.label(
                        f"Ausführungszeit: {tool_result.execution_time:.2f}s",
                    ...
                    if tool_result.status == "success":
                        ...
                        ui.label(f"Fehler: {tool_result.error_message}").classes(
        ...
        with self.content_container:
            ...
            if self.message.metadata and self.message.metadata.get("error_details"):
                with ui.expansion("Details anzeigen").classes("mt-2"):
                    ui.label(str(self.message.metadata["error_details"])).classes(
                        "text-xs",
                    )
        ...
        with self.metadata_container:
            with ui.row().classes("items-center justify-between"):
                ...
                if (
                    ...
                    and self.message.message_type == MessageType.TEXT
                ):
    ...
    def get_file_icon(self, file_type: str) -> str:
        ...
        if file_type in ["text/plain", "text/markdown", "text/html"]:
            return "description"
        if file_type in ["application/json", "application/xml"]:
            return "code"
        return "attach_file"
...
def create_text_message_bubble(
    content: str, is_user: bool = True, timestamp: datetime | None = None,
) -> MessageBubble:
    """
...
def create_file_message_bubble(
    filename: str, file_type: str, file_size: int, url: str | None = None,
) -> MessageBubble:
    """
