...
class AdvancedSettingsPage:
        ...
        with ui.element("div").classes("mb-6"):
            with ui.row().classes("items-center justify-between"):
                with ui.column():
                    ...
                    ui.label("Verwalte dein Profil und deine Einstellungen").classes(
                        "text-gray-600",
                    )
    ...
    def create_appearance_settings(self):
        ...
        with ui.element("div").classes("bg-white border rounded-lg p-6"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                # UI options
                self.compact_mode_switch = ui.switch("Kompakter Modus").classes(
                    "w-full",
                )
                self.high_contrast_switch = ui.switch("Hoher Kontrast").classes(
                    "w-full",
                )
                self.accessibility_mode_switch = ui.switch("Barrierefreiheit").classes(
                    "w-full",
                )
    ...
    def create_notification_settings(self):
        ...
        with ui.element("div").classes("bg-white border rounded-lg p-6"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                self.notifications_enabled_switch = ui.switch(
                    "Benachrichtigungen aktivieren",
                ).classes("w-full")

                # Email notifications
                self.email_notifications_switch = ui.switch(
                    "E-Mail-Benachrichtigungen",
                ).classes("w-full")

                # Push notifications
                self.push_notifications_switch = ui.switch(
                    "Push-Benachrichtigungen",
                ).classes("w-full")

                # Sound
                self.sound_enabled_switch = ui.switch("Sound aktivieren").classes(
                    "w-full",
                )
    ...
    def create_chat_settings(self):
        ...
        with ui.element("div").classes("bg-white border rounded-lg p-6"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                # Auto-save
                self.auto_save_switch = ui.switch("Auto-Save aktivieren").classes(
                    "w-full",
                )
    ...
    def create_admin_section(self):
        ...
        self.admin_container = ui.element("div").classes(
    ...
    def update_settings_values(self):
        ...
        if self.user_preferences:
            ...
            self.accessibility_mode_switch.value = (
                self.user_preferences.accessibility_mode
            )

        # Notification settings
        if self.user_preferences:
            self.notifications_enabled_switch.value = (
                self.user_preferences.notifications_enabled
            )
            self.email_notifications_switch.value = (
                self.user_preferences.email_notifications
            )
            self.push_notifications_switch.value = (
                self.user_preferences.push_notifications
            )
            self.sound_enabled_switch.value = self.user_preferences.sound_enabled

        # Chat settings
        if self.user_preferences:
            self.chat_history_limit_input.value = (
                self.user_preferences.chat_history_limit
            )
            self.auto_save_switch.value = self.user_preferences.auto_save
            if self.user_preferences.default_assistant_id:
                self.default_assistant_select.value = (
                    self.user_preferences.default_assistant_id
                )
    ...
    async def handle_password_change(
        self, current: str, new: str, confirm: str, dialog,
    ):
        ...
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.row().classes("justify-end space-x-2"):
                    ...
                    ui.button(
                        "Account löschen",
                        on_click=lambda: self.handle_delete_account(
                            confirm_text.value, dialog,
                        ),
                    ).classes("bg-red-600 text-white")
