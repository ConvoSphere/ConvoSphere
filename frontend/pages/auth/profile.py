    ...
    def create_profile_page(self):
        ...
        with ui.element("div").classes("w-full max-w-4xl mx-auto p-6"):
            ...
            with ui.element("div").classes("mb-8"):
                ui.html(
                    "<h1 style='font-size: 32px; font-weight: 700; color: #1f2937; margin-bottom: 8px;'>Profil bearbeiten</h1>",
                )
                ui.html(
                    "<p style='color: #6b7280; font-size: 16px;'>Verwalte deine persönlichen Einstellungen</p>",
                )
            ...
            with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-3 gap-8"):
                ...
                with ui.element("div").classes("lg:col-span-2"):
                    with ui.card().classes("p-6"):
                        ui.html(
                            "<h2 style='font-size: 20px; font-weight: 600; color: #1f2937; margin-bottom: 6px;'>Persönliche Informationen</h2>",
                        )
                        ui.html(
                            "<p style='color: #6b7280; font-size: 14px; margin-bottom: 24px;'>Aktualisiere deine persönlichen Daten</p>",
                        )
                        ...
                        self.save_button = ui.button(
                            ...
                            "bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors",
                        )
                ...
                with ui.element("div"):
                    with ui.card().classes("p-6"):
                        ui.html(
                            "<h2 style='font-size: 20px; font-weight: 600; color: #1f2937; margin-bottom: 6px;'>Einstellungen</h2>",
                        )
                        ui.html(
                            "<p style='color: #6b7280; font-size: 14px; margin-bottom: 24px;'>Anpassen der Anwendung</p>",
                        )

                        # Language selection
                        with ui.element("div").classes("mb-6"):
                            ui.label("Sprache").classes(
                                "block text-sm font-medium text-gray-700 mb-2",
                            )
                        ...
                        with ui.element("div").classes("mb-6"):
                            ...
                            self.email_notifications = ui.checkbox(
                                "E-Mail-Benachrichtigungen",
                            ).classes("text-sm text-gray-600 mb-2")
                            self.push_notifications = ui.checkbox(
                        ...
                        with ui.element("div").classes("border-t border-gray-200 pt-6"):
                            ui.html(
                                "<h3 style='font-size: 16px; font-weight: 600; color: #dc2626; margin-bottom: 12px;'>Gefahrenbereich</h3>",
                            )
                            ...
                            ui.button(
                                ...
                                "w-full bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors mb-2",
                            )
                            ...
                            ui.button(
                                ...
                                "w-full bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors",
                            )

            # Messages
            self.error_label = ui.label("").classes("text-red-600 text-sm hidden mt-4")
            self.success_label = ui.label("").classes(
                "text-green-600 text-sm hidden mt-4",
            )
    ...
    def change_password(self):
        ...
        try:
            ...
            with ui.dialog() as dialog, ui.card():
                ...
                current_password = ui.input("Aktuelles Passwort", password=True).props(
                    "type=password",
                )
                new_password = ui.input("Neues Passwort", password=True).props(
                    "type=password",
                )
                confirm_password = ui.input(
                    "Neues Passwort bestätigen", password=True,
                ).props("type=password")
                ...
                with ui.row():
                    ...
                    ui.button(
                        "Ändern",
                        on_click=lambda: self._handle_password_change(
                            dialog,
            ...
            ui.notify(
                f"Fehler beim Öffnen des Passwort-Dialogs: {str(e)}", type="negative",
            )

    async def _handle_password_change(
        self, dialog, current_password: str, new_password: str, confirm_password: str,
    ):
        ...
        try:
            ...
            if response.get("success"):
                ...
                ui.notify(
                    f"Fehler beim Ändern des Passworts: {response.get('error')}",
                    type="negative",
                )
        ...
        try:
            ...
            with ui.dialog() as dialog, ui.card():
                ...
                with ui.row():
                    ...
                    ui.button(
                        "Konto löschen",
                        on_click=lambda: self._handle_account_deletion(
                            dialog,
                            confirmation.value,
                        ),
            ...
            ui.notify(
                f"Fehler beim Öffnen des Lösch-Dialogs: {str(e)}", type="negative",
            )
    ...
    async def _handle_account_deletion(self, dialog, confirmation: str):
        ...
        try:
            if confirmation != "LÖSCHEN":
                ui.notify(
                    "Bitte geben Sie 'LÖSCHEN' ein, um zu bestätigen", type="negative",
                )
                return
            ...
            if response.get("success"):
                ...
                try:
                    from frontend.pages.auth.login import (
                        create_page as create_login_page,
                    )
                    ...
                    ui.notify(
                        f"Fehler beim Navigieren zur Login-Seite: {str(e)}",
                        type="negative",
                    )
            else:
                ui.notify(
                    f"Fehler beim Löschen des Kontos: {response.get('error')}",
                    type="negative",
                )
