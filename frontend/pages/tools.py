...
class AdvancedToolsPage:
        ...
        with ui.element("div").classes("mb-6"):
            with ui.row().classes("items-center justify-between"):
                with ui.column():
                    ...
                    ui.label(
                        "Verwalte und konfiguriere Tools für deine Assistenten",
    ...
    def create_tools_list(self):
        ...
        self.tools_container = ui.element("div").classes(
    ...
    def display_tools(self):
        ...
        if not filtered_tools:
            with self.tools_container:
                ui.label("Keine Tools gefunden").classes(
    ...
    def update_statistics(self):
        ...
        with self.stats_container:
            with ui.row().classes("grid grid-cols-1 md:grid-cols-4 gap-4"):
                ...
                with ui.element("div").classes("bg-white border rounded-lg p-4"):
                    ui.label("Gesamt").classes("text-sm text-gray-600")
                    ui.label(str(stats["total_tools"])).classes(
                ...
                with ui.element("div").classes("bg-white border rounded-lg p-4"):
                    ui.label("Aktiv").classes("text-sm text-gray-600")
                    ui.label(str(stats["active_tools"])).classes(
                ...
                with ui.element("div").classes("bg-white border rounded-lg p-4"):
                    ui.label("MCP Tools").classes("text-sm text-gray-600")
                    ui.label(str(stats["type_counts"].get("mcp", 0))).classes(
                ...
                with ui.element("div").classes("bg-white border rounded-lg p-4"):
                    ui.label("Ausführungen").classes("text-sm text-gray-600")
                    ui.label(str(stats["recent_executions"])).classes(
    ...
    def create_tool_card(self, tool: Tool):
        ...
        with ui.element("div").classes(
            ...
            with ui.row().classes("items-center justify-between mb-3"):
                ...
                ui.label(tool.status.value.title()).classes(
                    f"px-2 py-1 rounded text-xs {status_color}",
                )

            # Metadata
            with ui.row().classes(
                "items-center justify-between text-xs text-gray-500 mb-3",
            ):
            ...
            if tool.tags:
                with ui.row().classes("flex-wrap gap-1 mb-3"):
                    ...
                    if len(tool.tags) > 3:
                        ui.label(f"+{len(tool.tags) - 3}").classes(
        ...
        with ui.dialog() as dialog, ui.card().classes("w-full max-w-lg"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.row().classes("justify-end space-x-2"):
                    ...
                    ui.button("Hinzufügen", on_click=dialog.close).classes(
    ...
    def show_execute_tool_dialog(self, tool: Tool):
        ...
        with self.tool_info:
            ...
            ui.label(tool.description).classes("text-sm text-gray-600")
            ui.label(f"Typ: {tool.tool_type.value.title()}").classes(
            ...
            if result:
                ...
                with self.execution_results:
                    ...
                    if result.status == "success":
                        ...
                        ui.label(
                            f"Ausführungszeit: {result.execution_time:.2f}s",
                        ...
                        ui.label(f"Status: Fehler - {result.error_message}").classes(
                            "text-red-600",
                        )
    ...
    async def create_tool(
        ...
        try:
            ...
            if not validation["valid"]:
                ui.notify(
                    f"Tool-Validierung fehlgeschlagen: {validation['errors']}",
                    type="error",
                )
                return
            ...
            with ui.column().classes("space-y-4"):
                ...
                if tool.parameters:
                    with ui.expansion("Parameter", icon="settings"):
                        for param in tool.parameters:
                            with ui.row().classes(
                                "items-center space-x-4 p-2 bg-gray-50 rounded",
                            ):
                                ...
                                ui.label("✓" if param.required else "○").classes(
                                    "w-8 text-center",
                                )
