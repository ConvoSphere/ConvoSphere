...
class ConversationsPage:
    ...
    def create_conversations_interface(self):
        ...
        with ui.column().classes("h-screen flex flex-col"):
            ...
            self.loading_spinner = create_loading_spinner("Lade Konversationen...", size="lg")
    ...
    def create_page_header(self):
        ...
        with ui.element("div").classes("flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"):
            with ui.element("div").classes("flex items-center space-x-3"):
                ...
                with ui.element("div"):
                    ui.html("<h1 style='font-size: 24px; font-weight: 700; color: var(--color-text);'>Konversationen</h1>")
                    ui.html("<p style='font-size: 14px; color: var(--color-text-secondary);'>Chat-Verlauf und Nachrichten</p>")
    ...
    def create_search_and_filters(self):
        ...
        with ui.element("div").classes("p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"):
            with ui.element("div").classes("flex flex-col space-y-4"):
                ...
                with ui.element("div").classes("flex space-x-4"):
                    ...
                    ui.select(
                        "Assistent",
                        options=["alle"] + [assistant.get("name", "") for assistant in self.assistants],
                        value="alle",
        ...
        if not self.conversations:
            with self.conversations_container:
                with ui.element("div").classes("text-center py-8"):
                    ...
                    ui.html("<p style='font-size: 16px; color: var(--color-text-secondary);'>Keine Konversationen gefunden</p>")
                    ui.html("<p style='font-size: 14px; color: var(--color-text-secondary);'>Erstelle eine neue Konversation um zu beginnen</p>")
            return
    ...
    def filter_conversations(self) -> List[Dict[str, Any]]:
        ...
        if self.search_query:
            ...
            filtered = [
                ...
                if search_lower in conv.get("title", "").lower() or
                   search_lower in conv.get("assistant_name", "").lower()
            ]
        ...
        if self.selected_status_filter != "alle":
            ...
            filtered = [
                conv for conv in filtered
                if conv.get("is_active") == target_status
            ]
    ...
    def create_conversation_card(self, conversation: Dict[str, Any]):
        ...
        with ui.element("div").classes("bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md transition-shadow"):
            with ui.element("div").classes("flex items-center justify-between"):
                # Conversation info
                with ui.element("div").classes("flex-1"):
                    ui.html(f"<h3 style='font-size: 16px; font-weight: 600; color: var(--color-text);'>{conversation.get('title', 'Unbenannte Konversation')}</h3>")
                    ui.html(f"<p style='font-size: 14px; color: var(--color-text-secondary);'>Assistent: {conversation.get('assistant_name', 'Unbekannt')}</p>")
                    ui.html(f"<p style='font-size: 12px; color: var(--color-text-secondary);'>Erstellt: {format_relative_time(conversation.get('created_at'))}</p>")
                    ui.html(f"<p style='font-size: 12px; color: var(--color-text-secondary);'>Nachrichten: {conversation.get('message_count', 0)}</p>")
                ...
                with ui.element("div").classes("flex space-x-2"):
                    ...
                    if conversation.get("is_active"):
                        ui.button(
                            icon="archive",
                            on_click=lambda c=conversation: self.archive_conversation(c)
                        ).classes("w-8 h-8 bg-gray-500 text-white rounded")
                    else:
                        ui.button(
                            icon="unarchive",
                            on_click=lambda c=conversation: self.unarchive_conversation(c)
    ...
    def show_create_conversation_dialog(self):
        ...
        with ui.dialog() as dialog, ui.card():
            ui.html("<h2 style='font-size: 20px; font-weight: 600; color: var(--color-text); margin-bottom: 16px;'>Neue Konversation</h2>")
            
            # Assistant selection
            assistant_options = [assistant.get("name", "") for assistant in self.assistants]
            ...
            with ui.element("div").classes("flex justify-end space-x-2"):
                ...
                ui.button(
                    "Erstellen",
                    on_click=lambda: self.create_conversation(selected_assistant.value, title_input.value, dialog)
                ).classes("bg-blue-600 text-white")
    ...
    async def create_conversation(self, assistant_name: str, title: str, dialog):
        ...
        try:
            ...
            assistant = next((a for a in self.assistants if a.get("name") == assistant_name), None)
            if not assistant:
            ...
            ui.notify(f"Fehler beim Erstellen der Konversation: {str(e)}", type="negative")
    ...
    async def archive_conversation(self, conversation: Dict[str, Any]):
        ...
        try:
            success = await conversation_service.archive_conversation(conversation.get("id"))
            if success:
    ...
    async def unarchive_conversation(self, conversation: Dict[str, Any]):
        ...
        try:
            success = await conversation_service.unarchive_conversation(conversation.get("id"))
            if success:
                conversation["is_active"] = True
                self.display_conversations()
                ui.notify("Konversation wiederhergestellt", type="positive")
            else:
                ui.notify("Fehler beim Wiederherstellen der Konversation", type="negative")
        except Exception as e:
    ...
    async def delete_conversation(self, conversation: Dict[str, Any]):
        ...
        with ui.dialog() as dialog, ui.card():
            ui.html("<h3 style='font-size: 18px; font-weight: 600; color: var(--color-text); margin-bottom: 16px;'>Konversation löschen</h3>")
            ui.html(f"<p style='color: var(--color-text-secondary); margin-bottom: 16px;'>Möchtest du die Konversation '{conversation.get('title')}' wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.</p>")
            
            with ui.element("div").classes("flex justify-end space-x-2"):
                ui.button("Abbrechen", on_click=dialog.close).classes("bg-gray-500 text-white")
                ui.button(
                    "Löschen",
                    on_click=lambda: self.confirm_delete_conversation(conversation, dialog)
                ).classes("bg-red-600 text-white")
    
    async def confirm_delete_conversation(self, conversation: Dict[str, Any], dialog):
        """Confirm and delete conversation."""
        try:
            success = await conversation_service.delete_conversation(conversation.get("id"))
            if success:
                self.conversations = [c for c in self.conversations if c.get("id") != conversation.get("id")]
                self.display_conversations()
    ...
    async def connect_websocket(self):
        ...
        try:
            ...
            websocket_service.on_message("conversation_updated")(self.handle_conversation_update)
            websocket_service.on_message("conversation_created")(self.handle_conversation_created)
            websocket_service.on_message("conversation_deleted")(self.handle_conversation_deleted)
