...
class MCPToolsPage:
        ...
        with ui.column().classes("w-full h-full p-4 gap-4"):
            ...
            with ui.row().classes("w-full justify-between items-center"):
                ...
                with ui.row().classes("gap-2"):
                    ui.button("Add Server", on_click=self.show_add_server_dialog).classes("bg-blue-500")
            ...
            with ui.row().classes("w-full h-full gap-4"):
                ...
                with ui.column().classes("w-1/3 bg-gray-50 p-4 rounded-lg"):
                    ...
                    # Add server button
                    ui.button("+ Add New Server", on_click=self.show_add_server_dialog).classes("w-full mt-4 bg-green-500")
    ...
    def create_add_server_dialog(self):
        ...
        with ui.dialog() as self.server_dialog, ui.card().classes("w-96"):
            ...
            with ui.row().classes("w-full justify-end gap-2"):
                ui.button("Cancel", on_click=self.server_dialog.close).classes("bg-gray-500")
                ui.button("Add Server", on_click=lambda: self.add_server(
                    server_id.value,
                    server_url.value,
                    server_name.value
                )).classes("bg-blue-500")
    ...
    def create_tool_execute_dialog(self):
        ...
        with ui.dialog() as self.tool_execute_dialog, ui.card().classes("w-96"):
            ...
            self.tool_description_label = ui.label("").classes("text-sm text-gray-600 mb-4")
            
            self.arguments_container = ui.column().classes("w-full mb-4")
            
            with ui.row().classes("w-full justify-end gap-2"):
                ui.button("Cancel", on_click=self.tool_execute_dialog.close).classes("bg-gray-500")
    ...
    async def load_data(self):
        ...
        try:
            ...
            if tools_response.success:
                self.tools = [
                    MCPTool(**tool_data) for tool_data in tools_response.data
                ]
    ...
    def update_servers_grid(self):
        ...
        for server in self.servers:
            with self.servers_grid:
                with ui.card().classes("w-full p-4 cursor-pointer hover:bg-blue-50").on("click", lambda s=server: self.select_server(s)):
                    with ui.row().classes("w-full justify-between items-start"):
                        with ui.column().classes("flex-1"):
                            ...
                            with ui.row().classes("gap-4 mt-2"):
                                status_color = "text-green-600" if server.is_connected else "text-red-600"
                                status_text = "Connected" if server.is_connected else "Disconnected"
                                ui.label(f"● {status_text}").classes(f"text-sm {status_color}")
                                
                                ui.label(f"Tools: {server.tool_count}").classes("text-sm text-gray-600")
                                ui.label(f"Resources: {server.resource_count}").classes("text-sm text-gray-600")
                        
                        with ui.column().classes("gap-1"):
                            ui.button("Remove", on_click=lambda s=server: self.remove_server(s.server_id)).classes("bg-red-500 text-white text-xs")
    ...
    def update_tools_grid(self):
        ...
        for tool in self.tools:
            with self.tools_grid:
                with ui.card().classes("w-full p-4 cursor-pointer hover:bg-green-50").on("click", lambda t=tool: self.select_tool(t)):
                    with ui.row().classes("w-full justify-between items-start"):
                        with ui.column().classes("flex-1"):
                            ui.label(tool.name).classes("font-semibold")
                            ui.label(tool.description).classes("text-sm text-gray-600 mt-1")
                            
                            with ui.row().classes("gap-4 mt-2"):
                                ui.label(f"Server: {tool.server_name}").classes("text-sm text-gray-600")
                                ui.label(f"Category: {tool.category}").classes("text-sm text-gray-600")
                                ui.label(f"Parameters: {len(tool.parameters)}").classes("text-sm text-gray-600")
                        
                        with ui.column().classes("gap-1"):
                            ui.button("Execute", on_click=lambda t=tool: self.show_execute_tool_dialog(t)).classes("bg-blue-500 text-white text-xs")
        ...
        try:
            ...
            if response.success:
                ...
                with ui.dialog() as result_dialog, ui.card().classes("w-96"):
                    ui.label("Tool Execution Result").classes("text-lg font-semibold mb-4")
