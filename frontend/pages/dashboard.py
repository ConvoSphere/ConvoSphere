...
from components.common.loading_spinner import (
...
from utils.helpers import format_number, format_relative_time
...
class DashboardPage:
    ...
    def create_dashboard(self):
        ...
        with ui.column().classes("w-full max-w-7xl mx-auto p-6 space-y-6"):
            ...
            with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-3 gap-6"):
                ...
                # Quick Actions
                self.quick_actions_container = ui.element("div").classes(
                    "lg:col-span-1",
                )
    ...
    def create_stat_card(
        self, title: str, value: str, subtitle: str, icon: str, color_class: str,
    ):
        ...
        with ui.card().classes("p-4"):
            with ui.row().classes("items-center justify-between"):
                ...
                ui.icon(icon).classes(
                    f"w-8 h-8 {color_class} text-white rounded-lg p-1",
                )

    def create_quick_actions(self):
        """Create quick actions panel."""
        with self.quick_actions_container:
            ui.label("Schnellaktionen").classes(
    ...
    def create_recent_conversations(self):
        ...
        with self.recent_conversations_container:
            ui.label("Letzte Konversationen").classes(
                "text-lg font-semibold text-gray-900 mb-4",
            )
        ...
        with ui.card().classes("p-4 hover:shadow-md transition-shadow cursor-pointer"):
            with ui.row().classes("items-center justify-between"):
                with ui.element("div").classes("flex-1"):
                    ...
                    ui.label(f"mit {conversation.assistant_name}").classes(
                        "text-sm text-gray-600",
                    )

                    if conversation.last_message:
                        ui.label(
                            conversation.last_message[:100] + "..."
                            if len(conversation.last_message) > 100
                            else conversation.last_message,
                        ).classes("text-sm text-gray-500 mt-1")

                    ui.label(format_relative_time(conversation.updated_at)).classes(
                        "text-xs text-gray-400 mt-1",
                    )

                with ui.element("div").classes("text-right"):
                    ui.label(
                        f"{format_number(conversation.message_count)} Nachrichten",
