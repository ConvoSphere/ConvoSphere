    ...
    def create_assistants_page(self):
        ...
        with ui.column().classes("w-full max-w-7xl mx-auto p-6 space-y-6"):
            ...
            self.loading_spinner = create_loading_spinner(
                "Lade Assistenten...", size="lg",
            )
            self.error_component = create_error_message("", dismissible=True)

            # Assistants Grid
            self.assistants_container = ui.element("div").classes(
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.row().classes("items-center space-x-2"):
                    ...
                    ui.slider(
                        ...
                        step=0.1,
                        value=0.7,
                        on_change=lambda e: self.update_form_data(
                            "temperature", e.value,
                        ),
                    ).classes("flex-1")
        ...
        with ui.card().classes("p-4 hover:shadow-md transition-shadow"):
            with ui.column().classes("space-y-3"):
                ...
                with ui.row().classes("items-center justify-between"):
                    ...
                    ui.label(assistant.status.title()).classes(
                        f"px-2 py-1 rounded-full text-xs font-medium {status_color}",
                    )
                ...
                ui.label(f"{tools_count} Tools zugewiesen").classes(
