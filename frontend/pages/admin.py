...
from services.user_service import user_service, UserProfile, UserRole, UserStatus
...
class AdvancedAdminPage:
        ...
        with ui.element("div").classes("mb-6"):
            with ui.row().classes("items-center justify-between"):
                with ui.column():
                    ...
                    ui.label("System-Verwaltung und Benutzer-Management").classes("text-gray-600")
    ...
    def create_navigation(self):
        ...
        with ui.element("div").classes("mb-6"):
            ...
            with ui.tab_panels(tabs, value=self.current_tab).classes("w-full") as panels:
                # Users panel
    ...
    def create_users_table(self):
        ...
        with ui.element("div").classes("bg-white border rounded-lg overflow-hidden"):
            ...
            with ui.element("div").classes("bg-gray-50 px-6 py-3 border-b"):
                with ui.row().classes("grid grid-cols-12 gap-4 text-sm font-medium text-gray-700"):
    ...
    def create_activity_statistics(self):
        ...
        with ui.element("div").classes("bg-white border rounded-lg p-6"):
            ...
            with ui.column().classes("space-y-4"):
                ...
                with ui.element("div"):
                    ui.label("Neue Registrierungen (letzte 30 Tage)").classes("font-medium mb-2")
        ...
        with ui.element("div").classes("px-6 py-4 hover:bg-gray-50"):
            with ui.row().classes("grid grid-cols-12 gap-4 items-center"):
                ...
                role_config = {
                    ...
                    UserRole.MODERATOR: {"color": "text-orange-600", "text": "Moderator"},
                    UserRole.USER: {"color": "text-blue-600", "text": "User"}
                }
                config = role_config.get(user.role, role_config[UserRole.USER])
                ui.label(config["text"]).classes(f"col-span-2 text-sm font-medium {config['color']}")
                ...
                status_config = {
                    ...
                    UserStatus.PENDING: {"color": "text-yellow-600", "text": "Ausstehend"}
                }
                config = status_config.get(user.status, status_config[UserStatus.ACTIVE])
                ui.label(config["text"]).classes(f"col-span-2 text-sm font-medium {config['color']}")
    ...
    async def add_user(self, first_name: str, last_name: str, username: str, email: str, password: str, role: str, dialog):
        """Add new user."""
