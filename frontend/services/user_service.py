...
class UserService:
    ...
    async def get_current_user(self, force_refresh: bool = False) -> Optional[UserProfile]:
        """
    ...
    async def update_profile(self, profile_data: Dict[str, Any]) -> Optional[UserProfile]:
        """
        Update user profile.
    ...
    async def upload_avatar(self, avatar_data: bytes, filename: str) -> Optional[str]:
        """
        Upload user avatar.
    ...
    async def get_user_stats(self) -> Optional[UserStats]:
        """
        Get user statistics.
    ...
    async def get_all_users(self, skip: int = 0, limit: int = 100) -> List[UserProfile]:
        ...
        try:
            ...
            if response.success and response.data:
                self.users = [self._create_user_profile_from_data(user_data) for user_data in response.data]
                return self.users
            else:
    ...
    def is_moderator(self) -> bool:
        ...
        return self.current_user and self.current_user.role in [UserRole.ADMIN, UserRole.MODERATOR]
    ...
    def can_manage_user(self, target_user: UserProfile) -> bool:
        ...
        if self.current_user.role == UserRole.MODERATOR and target_user.role == UserRole.USER:
            return True
    ...
    def search_users(self, query: str) -> List[UserProfile]:
        ...
        return [
            ...
            if query_lower in u.username.lower() or 
               ...
               query_lower in f"{u.first_name} {u.last_name}".lower()
        ]
    ...
    def _create_user_preferences_from_data(self, data: Dict[str, Any]) -> UserPreferences:
