...
class ConversationService:
    ...
    async def create_conversation(
        self, assistant_id: str, title: str | None = None,
    ) -> Conversation | None:
        """
    ...
    async def get_conversation_messages(
        ...
        try:
            ...
            if response.success and response.data:
                ...
                return messages
            handle_api_error(
                response, f"Laden der Nachrichten für Konversation {conversation_id}",
            )
            return []

        except Exception as e:
            handle_network_error(
                e, f"Laden der Nachrichten für Konversation {conversation_id}",
            )
            return []

    async def send_message(self, conversation_id: str, content: str) -> Message | None:
        """
    ...
    async def archive_conversation(self, conversation_id: str) -> bool:
        ...
        try:
            ...
            if not response.get("success"):
                raise Exception(
                    f"Failed to archive conversation: {response.get('error')}",
                )
    ...
    async def delete_conversation(self, conversation_id: str) -> bool:
        ...
        try:
            ...
            if not response.get("success"):
                raise Exception(
                    f"Failed to delete conversation: {response.get('error')}",
                )
    ...
    def search_conversations(self, query: str) -> list[Conversation]:
        ...
        for conversation in self.conversations:
            ...
            if (
                ...
                and query_lower in conversation.last_message.lower()
            ):
                matching_conversations.append(conversation)
                continue
    ...
    def get_conversation_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_messages_per_conversation": total_messages / total
            if total > 0
            else 0,
        }
