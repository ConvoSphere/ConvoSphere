...
class FileService:
    ...
    def validate_file(
        ...
        if len(file_data) > self.max_file_size:
            errors.append(
                f"File size exceeds maximum limit of {format_file_size(self.max_file_size)}",
            )
            ...
            with open(file_path, "rb") as f:
                while True:
                    ...
                    async with http_client.session.post(
                        ...
                        data=chunk_form,
                        headers={
                            "Authorization": http_client.headers.get(
                                "Authorization", "",
                            ),
                        },
                    ) as response:
    ...
    def get_upload_progress(self, file_id: str) -> UploadProgress | None:
    ...
    def clear_completed_uploads(self):
        ...
        completed_ids = [
            file_id
            for file_id, progress in self.uploads.items()
            if progress.status
            in [FileStatus.COMPLETED, FileStatus.FAILED, FileStatus.CANCELLED]
        ]
