...
class PerformanceManager:
    ...
    def cache_clear(self, pattern: str | None = None):
        """Clear cache entries."""
        if pattern:
    ...
    def get_metrics_summary(
        self, metric: PerformanceMetric | None = None,
    ) -> dict[str, Any]:
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        total_requests = len(
            [
                ...
                if m.metric
                in [PerformanceMetric.CACHE_HIT, PerformanceMetric.CACHE_MISS]
            ],
        )
        cache_hits = len(
            [m for m in self.metrics if m.metric == PerformanceMetric.CACHE_HIT],
        )
...
def cache_result(ttl: int | None = None, key_func: Callable | None = None):
    """Convenience decorator for result caching."""
    return performance_manager.cache_result(ttl, key_func)


def record_metric(
    metric: PerformanceMetric, value: float, context: dict[str, Any] | None = None,
):
